"""Scoped native pairs using unchanged Audit and real UI; optional fresh output."""
import argparse
import json
import os
...
parser=argparse.ArgumentParser();parser.add_argument('--scene',required=True,choices=['engine_bay','shelf_picking','dig_site']);parser.add_argument('--out',type=Path);args=parser.parse_args()
args.api='http://127.0.0.1:8750';args.viewer='http://127.0.0.1:8780'
out=args.out or Path(os.environ['RUN_DIR'])/'changed_pairs'/args.scene;environment,blender=check_environment(out);out.mkdir(parents=True,exist_ok=False)
