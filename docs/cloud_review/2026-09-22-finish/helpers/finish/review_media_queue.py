...
Run through record.py review_media_queue01 after the earlier review helper ends.
...
def main():
    ...
    log_path = run / 'review_media_queue01.jsonl'
    summary_path = run / 'review_media_queue01.json'
    ...
    def completed_services():
        return [name for name in ('model_resume01', 'api_resume01', 'viewer_resume01')
    ...
    def discover():
        ...
        for execution in sorted(run.glob('produce_video_*_command/execution.json')):
            ...
            for scene in scenes_argument(argv):
                ...
                if scene in complete:
                    raise RuntimeError('Multiple successful productions for one scene: ' + scene)
                complete[scene] = {'manifest': str(manifest_path), 'command_record': str(execution)}
                if actual['bytes'] >= helper.LIMIT:
    ...
    log('started', helper_sha256=helper_sha, interval_seconds=INTERVAL_SECONDS,
        generation='resume01', scope='Closed originals/full productions only; no service operations or failed retries')
    try:
        while True:
            ...
            last_observation = {'complete_scenes': sorted(complete), 'missing_scenes': missing,
                ...
                'completed_resume01_service_records': stopped}
