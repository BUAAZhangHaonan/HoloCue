...
def load_source_phases(root, run):
    ...
    for phase in manifest['phases']:
        ...
        snapshot = read(path)
        digest = phase['source_digest']
        if snapshot['digest'] != digest or not snapshot['files'] or digest in phases:
            raise ValueError('Invalid or duplicate source phase snapshot')
        phases[digest] = {**phase, 'snapshot_path': path, 'snapshot': snapshot}
...
def main():
    ...
    def current_result(scene, kind):
        ...
        for base, entry in pair_outputs.items():
            ...
            if path.is_file():
                ...
                result.update(status='interrupted_without_final_report', report_exists=False,
    ...
    coverage = {'source_digest': source['digest'], 'asset_digest': assets['digest'],
        ...
        'final_source_checks': {name: identity(name) for name in ('pytest_camera_final', 'camera_atomic_live', 'pairs_shelf_picking_attempt02')},
    ...
    for old_scene in old_coverage['scenes']:
        ...
        if scene == 'shelf_picking' and isinstance(row['targeted_pairs_this_run'], dict):
            matches = [attempt for attempt in row['targeted_pairs_this_run']['attempts']
                       if attempt['execution']['run_id'] == 'pairs_shelf_picking_attempt02']
            if len(matches) != 1:
                raise RuntimeError('Final Shelf attempt lacks unique actual output evidence')
            coverage['final_source_checks']['pairs_shelf_picking_attempt02']['result'] = matches[0]
