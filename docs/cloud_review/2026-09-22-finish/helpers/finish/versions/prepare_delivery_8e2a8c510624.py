...
def load_source_phases(root, run):
    ...
    for phase in manifest['phases']:
        ...
        if sha256(path) != phase['source_snapshot_sha256']:
    ...
    if sha256(previous) != expected['sha256'] or previous.stat().st_size != expected['bytes']:
        raise ValueError('Preserved original camera source differs from the old snapshot')
    return manifest, phases


def stable_execution_phase(record, phases, assets):
    digest = record['source_before']['digest']
    if (digest not in phases or record['source_after']['digest'] != digest
            or record['assets_before']['digest'] != assets or record['assets_after']['digest'] != assets
            or 'finished_at' not in record or 'returncode' not in record):
        raise RuntimeError('Execution is unfinished, crosses source phases or changes assets')
    return digest


def pair_output_owners(commands, root, run):
    outputs = {}
    for name, record in commands.items():
        argv = record['command']
        if not {Path(x).name for x in argv} & {'capture_changed_pairs.py', 'capture_changed_pairs_attempt.py'}:
            continue
        scene = option(argv, '--scene')
        if scene not in SCENES:
            raise RuntimeError('Pair execution lacks a valid explicit scene')
        value = option(argv, '--out')
        out = Path(value) if value else run / 'changed_pairs' / scene
        cwd = Path(record['cwd'])
        cwd = cwd if cwd.is_absolute() else root / cwd
        out = out if out.is_absolute() else cwd / out
        if '..' in out.parts or not out.is_relative_to(run / 'changed_pairs') or out in outputs:
            raise RuntimeError('Pair output must have a unique explicit RUN4 owner')
        outputs[out] = {'owner': name, 'scene': scene}
    return outputs
...
def main():
    ...
    if phase_manifest['current_source_digest'] != source['digest'] or phase_manifest['asset_digest'] != assets['digest']:
        raise RuntimeError('Explicit phase boundary differs from current source/assets')
    if source_phases[source['digest']]['snapshot']['files'] != source['files']:
        raise RuntimeError('Final source snapshot differs from its frozen camera phase')
    video_source = next(d for d, phase in source_phases.items() if phase['phase_id'] == 'before_camera_atomic')
    for name, record in commands.items():
        stable_execution_phase(record, source_phases, assets['digest'])
    ...
    selection = {'package_id': 'HoloCue_Cloud_Finish90_20260921', 'privacy_reviewed': False,
                 ...
                 'note': 'RUN4 final continuation; preserve phase-specific failures and identities. '
                         ...
                         'RUN2 ZIP-sealed, RUN3 write-once inventory; RUN4 commands retain their explicit stable source phase.'}
    ...
    for name, record in commands.items():
        ...
        selection['runs'].append({'run_id': name, 'scope': 'current' if current else 'historical',
            ...
            'command_record': path, 'report': path, 'source_digest': digest,
            'note': f"Actual return code {record['returncode']}. UI capture/production is UI-only; "
                    f"Source phase {source_phases[digest]['phase_id']}; no historical rerun is claimed. "
    ...
    if local_inventory is not None and (local_inventory.get('passed') is not True
            or local_inventory['source_digest'] != video_source or local_inventory['asset_digest'] != assets['digest']
            or local_inventory['current_source_digest'] != source['digest']):
        raise RuntimeError('Local original-media delivery inventory is incomplete or has different identities')
    if local_inventory is not None and local_inventory['source_phases']['sha256'] != sha256(run / 'SOURCE_PHASES.json'):
        raise RuntimeError('Local video inventory is bound to a different source phase manifest')
    ...
    def successful_record(value):
        ...
        if name is None or commands[name]['returncode'] != 0:
            raise RuntimeError('Review copy prerequisite lacks successful stable-phase execution: ' + rel)
        return name
    ...
    def identity(name):
        ...
        return {'run_id': name, 'command_record': relative(run / (name + '_command') / 'execution.json'),
                'scope': 'current' if record['source_before']['digest'] == source['digest'] else 'historical',
                'source_phase': source_phases[record['source_before']['digest']]['phase_id'],
    ...
    pair_outputs = pair_output_owners(commands, root, run)
    for name, record in commands.items():
        argv = record['command']
    ...
    for out, capture in captures.items():
        ...
        if before:
            ...
            for phase in [before] + ([after] if after else []):
                if phase['source']['digest'] != commands[owner]['source_before']['digest'] or phase['assets']['digest'] != assets['digest']:
        ...
        for scene in capture['scenes']:
            ...
            expected = {'source_digest': commands[owner]['source_before']['digest'], 'asset_digest': assets['digest'],
            ...
            for media_name, producer in producer_entries:
                ...
                if producer_record['source_before']['digest'] != expected['source_digest']:
                    raise RuntimeError('Scene production execution differs from its capture source phase')
                ...
                if accepted_here and expected['source_digest'] != video_source:
                    raise RuntimeError('Accepted twelve-scene media must retain the pre-camera source phase')
    ...
    if args.intro_collection_manifest:
        ...
        if (collection.get('source_digest') != video_source or collection.get('asset_digest') != assets['digest']
                or collection.get('source_phase_manifest_sha256') != sha256(run / 'SOURCE_PHASES.json')):
            raise RuntimeError('Collection input media/source phase boundary differs')
        ...
        for output in collection['outputs']:
            ...
            for item in sources:
                if item.get('source_digest') != video_source or item.get('asset_digest') != assets['digest']:
                    raise RuntimeError('Collection source input phase differs from accepted videos')
            ...
            if collection.get('execution_source_digest') != commands[matching[0]]['source_before']['digest']:
                raise RuntimeError('Collection execution phase differs from its actual command')
    ...
    top_owner = {'pytest_final.xml': 'pytest_final', 'pytest_camera_final.xml': 'pytest_camera_final', 'viser_opacity_install.json': 'install_visor_opacity_fix', 'readiness.json': 'readiness', 'service_shutdown.json': 'shutdown',
    ...
    for path in sorted(run.rglob('*')):
        ...
        if rel.parts[0] == 'videos':
            ...
            matches = [entry['owner'] for out, entry in pair_outputs.items() if path.is_relative_to(out)]
            if len(matches) != 1:
                raise RuntimeError('Pair evidence has no unique recorded output owner: ' + relative(path))
            owner = matches[0]
    ...
    for path in sorted((root / '.work/cloud_finish90_20260921/versions').glob('*.py')):
        add(path, note='Exact preserved delivery helper bytes from the pre-camera source phase.')
    camera_directory = root / '.work/cloud_finish90_20260921/camera_fix'
    for path in sorted(camera_directory.rglob('*')):
        if path.is_file() and '__pycache__' not in path.parts and path.suffix in {'.py', '.json', '.md', '.patch', '.diff'}:
            add(path, 'delivery', 'Explicit camera repair source/helper/patch evidence; phase snapshots bind old and new application bytes.')
    ...
    def current_result(scene, kind):
        ...
        attempts = []
        for base, entry in pair_outputs.items():
            if entry['scene'] != scene:
                continue
            name = entry['owner']
            path = base / 'report.json'
            result = {'execution': identity(name), 'output_directory': relative(base), 'passed': False}
            if path.is_file():
                report = read(path)
                if not isinstance(report, dict) or report.get('scene_id') != scene:
                    raise RuntimeError('Pair report scene differs from recorded --scene')
                result.update(report_path=relative(path), report=report,
                              passed=report.get('passed') is True and commands[name]['returncode'] == 0)
            else:
                result.update(status='interrupted_without_final_report' if base.is_dir() else 'no_output_directory', report_exists=False,
                              note='No camera/native outcome is inferred from a missing report; read the actual command and stderr for any preflight rejection.',
                              completed_images=[relative(p) for p in sorted(base.rglob('*.png'))])
            attempts.append(result)
        if not attempts:
            return 'not_run'
        return {'attempts': attempts, 'passed_in_at_least_one_recorded_phase': any(x['passed'] for x in attempts),
                'passed_on_final_source': any(x['passed'] and x['execution']['scope'] == 'current' for x in attempts),
                'note': 'Every explicit output attempt and original failure retained with its actual source phase.'}
    ...
    coverage = {'source_digest': source['digest'], 'asset_digest': assets['digest'],
        'source_phases': {'manifest': relative(run / 'SOURCE_PHASES.json'),
                          'sha256': sha256(run / 'SOURCE_PHASES.json'), 'definition': phase_manifest},
        ...
        'before_camera_atomic_checks': {name: identity(name) for name in ('pytest_final', 'live_final', 'viewer_response_final', 'response_final')},
        'final_source_checks': {name: identity(name) for name in ('pytest_camera_final', 'camera_atomic_live', 'pairs_shelf_picking_attempt03')},
        'visual_acceptance': acceptance,
        'ui_video_coverage': {'scope': 'Real Viser UI-only under the pre-camera source; not rerun under final camera source and no Blender/native acceptance',
                              'source_digest': video_source, 'source_scope': 'historical', 'scenes': []}}
    for name in coverage['before_camera_atomic_checks']:
        if name not in commands or commands[name]['source_before']['digest'] != video_source:
            raise RuntimeError('Pre-camera check has an unexpected source phase: ' + name)
    for name in coverage['final_source_checks']:
        if name not in commands or commands[name]['source_before']['digest'] != source['digest']:
            raise RuntimeError('Final camera check missing or has an unexpected source phase: ' + name)
    camera_out = absolute(option(commands['camera_atomic_live']['command'], '--out'),
                          absolute(commands['camera_atomic_live']['cwd']))
    if not camera_out.is_relative_to(run):
        raise RuntimeError('Camera live result escaped current run')
    camera_result = camera_out / 'result.json'
    if camera_result.is_file():
        coverage['final_source_checks']['camera_atomic_live'].update(
            report_path=relative(camera_result), report_sha256=sha256(camera_result), report=read(camera_result))
    else:
        coverage['final_source_checks']['camera_atomic_live']['status'] = 'interrupted_without_final_report'
    for old_scene in old_coverage['scenes']:
        ...
        if scene == 'shelf_picking' and isinstance(row['targeted_pairs_this_run'], dict):
            matches = [attempt for attempt in row['targeted_pairs_this_run']['attempts']
                       if attempt['execution']['run_id'] == 'pairs_shelf_picking_attempt03']
            if len(matches) != 1:
                raise RuntimeError('Final Shelf attempt lacks unique actual output evidence')
            coverage['final_source_checks']['pairs_shelf_picking_attempt03']['result'] = matches[0]
    ...
    for name, group in [('pytest_final', 'before_camera_atomic_checks'), ('pytest_camera_final', 'final_source_checks')]:
        path = run / (name + '.xml')
        cases = list(ET.parse(path).getroot().iter('testcase'))
        coverage[group][name]['junit'] = {'path': relative(path), 'tests': len(cases),
    ...
    add(absolute(phase_manifest['previous_file']), 'delivery',
        'Exact pre-camera viewer_scene source bytes, bound to the historical phase snapshot; not final application source.')
