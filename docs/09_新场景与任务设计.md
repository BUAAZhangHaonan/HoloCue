...
通用约定沿用 v1:米制、Z 向上、右手系;rotate 以对象局部 Z 轴为正、逆时针为正(顺时针输入在 fixture 中记为负角);锚点书写在**参照对象局部坐标系**(见 `src/holocue/projection.py` 的 goal 计算式);交互对象能力与锚点写入场景 JSON;环境对象进入新增的 `environment` 列表(仅渲染,不进入 LLM 提示词与校验);每场景附带 v1 式扩展测试输入表。v2 场景的 Viser 视图默认开启 `render_hints.fit_camera`(按场景包围盒自动适配透视取景),Blender 正交取景以场景 JSON 的 `ortho_scale_m` 为准,两者口径差异属预期。
...
| 把 NODE3 拧一下 | NODE3 无 rotate 能力 → 校验层拒绝(对应 v1"C 转 30 度"用例) |
...
v1 三场景(旋钮/插件/积木)验证接口闭环;v2 五场景在完全不同的世界(机房/维修台/仓储/光学台/田野)复用同一 schema、同一 planner 提示词、同一 N/σ 策略,不改变研究定义。v2 沿视轴调焦距离差为 0.4–1.7m(视点声明见各场景),对象数 5–8(含消歧项),均高于 v1。五场景任务图刻意差异化:单 precise+深查(rack)、双 precise(drone)、双 background+同深度角色对照(shelf)、跨层内禀注意+光学主题(bench)、非插入类 current+neutral(dig)。
...
2. Blender 渲染 ≥1280×900(按各自 render_hints 取景),**三轮独立视觉审核**通过:几何正确性(对象可辨、锚点几何合理)、任务可读性(深度分层与角色在画面中可辨)、美观与丰富度不低于 v1 水平。
