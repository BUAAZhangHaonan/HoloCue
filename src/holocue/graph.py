...
def build_graph(planner):
    ...
    async def plan(state):
        d, trace = await planner.decide(
            Session.model_validate(state["session"]),
    ...
    def ground(state):
        try:
            validate_decision(
                Decision.model_validate(state["decision"]), SceneSpec.model_validate(state["scene"])
            )
    ...
    g.add_node("interpret", plan)
    g.add_node("ground", ground)
    g.add_edge(START, "interpret")
    g.add_edge("interpret", "ground")
...
class GraphPlanner:
    def __init__(self, planner):
        self.graph = build_graph(planner)
        self.mode = planner.mode

    async def decide(self, s, scene, msg):
        result = await self.graph.ainvoke(
            {"session": s.model_dump(), "scene": scene.model_dump(), "message": msg.model_dump()},
            {"recursion_limit": 6},
        )
