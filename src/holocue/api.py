...
def create_app(service=None) -> FastAPI:
    if service is None:
        ...
        if mode != "live":
            raise ValueError("HOLOCUE_MODE must be live")
        provider = OpenAICompatiblePlanner()
        state_path = Path(
            os.environ.get("HOLOCUE_STATE_PATH", str(root() / "runs/simulation/state.sqlite"))
        ).resolve()
    ...
    async def domain(req, e):
        return JSONResponse(status_code=422, content={"detail": str(e)})

    @app.exception_handler(KeyError)
    async def missing(req, e):
        return JSONResponse(status_code=404, content={"detail": str(e)})

    @app.exception_handler(FileNotFoundError)
    async def file_missing(req, e):
        return JSONResponse(status_code=404, content={"detail": "scene/resource not found"})

    @app.get("/health")
    def health():
        return {
            "status": "ok",
            "backend_mode": service.planner.mode,
            "renderer_kind": "semantic_preview",
            "schema_version": "1.1",
        }
    ...
    def session(sid: str):
        return service.store.get(sid)

    @app.post("/api/v1/sessions/{sid}/messages", status_code=202)
    async def message(sid: str, body: UserMessage):
        return await service.submit(sid, body)

    @app.get("/api/v1/jobs/{jid}")
    def job(jid: str):
        return service.store.job(jid)

    @app.get("/api/v1/sessions/{sid}/display")
    def display(sid: str):
        return service.display(sid)
    ...
    def events(sid: str):
        return service.store.events(sid)
    ...
    def control(sid: str, operation: str, body: RevisionRequest):
        if operation not in ("pause", "resume", "complete"):
            raise HTTPException(422, "unknown control operation")
