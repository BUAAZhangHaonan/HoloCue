...
EXECUTION_TEXT = {
    'idle': '等待任务', 'running': '任务执行中', 'paused': '任务已暂停',
    'planning': '正在理解指令', 'error': '执行出现错误',
}
...
def object_summary(spec: SceneSpec, state: Session, object_id: str) -> ObjectSummary:
    ...
    suspended = [task for group in state.suspended for task in group
                 if task.semantic.target_id == object_id]
    ...
    if queued:
        ...
        status = (EXECUTION_TEXT[state.execution] if first.task_role == 'current'
                  else ROLE_TEXT[first.task_role])
    elif suspended:
        status = '等待恢复'
    elif occupants:
        status = '已有放置对象'
...
def session_status(spec: SceneSpec, state: Session) -> str:
    ...
    if state.execution == 'idle' and state.suspended:
        ...
        execution = '当前计划已完成'
    return (f'**{spec.title}**\n\n{execution}\n\n'
            f'模型模式 {state.backend_mode}　任务版本 {state.revision}')
