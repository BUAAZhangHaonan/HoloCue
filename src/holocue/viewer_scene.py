...
def selected_cue_view(
    spec, packet, current_poses, elapsed, object_id, mode, position, look_at, up, aspect, brightness=1.0
):
    ...
    inspection = mode == "inspection" or any(
        c.target_id == object_id and c.action == "inspect_back" for c in packet.cues
    )
    ...
    for cue in packet.cues:
        if cue.target_id != object_id or cue.n_gaussians <= 0:
            continue
        pose = trajectory(cue, elapsed.get(cue.task_id, 0.0))
        ...
        calibration = profile()
        nominal_width, _, _ = envelope(cue.sigma_value, 0.0, calibration)
    ...
    if mode == "inspection" or (
        mode == "detail" and first_cue is not None and first_cue.action == "inspect_back"
    ):
    ...
    return fit(
        np.concatenate([target_points, points]),
        -forward,
        spec.render_hints.fov_y_deg,
        aspect,
        spec.render_hints.viewport_margin,
        up,
    )
...
class SceneRenderer:
    def __init__(self, client: viser.ClientHandle, spec: SceneSpec):
        ...
        for p in spec.environment:
            ...
            parent = scene.add_frame(
                path, show_axes=False, position=p.pose.position_m, wxyz=p.pose.wxyz, scale=p.scale_m
            )
            child = scene.add_glb(
                path + "/asset",
                resource(root(), p.asset).read_bytes(),
                wxyz=correction,
                cast_shadow=True,
                receive_shadow=True,
            )
        ...
        for obj in spec.objects:
            ...
            child = scene.add_glb(
                path + "/asset",
                resource(root(), obj.asset).read_bytes(),
                wxyz=correction,
                cast_shadow=True,
                receive_shadow=True,
            )
            ...
            point = np.array(
                [
                    (local.bounds[0, 0] + local.bounds[1, 0]) / 2,
                    (local.bounds[0, 1] + local.bounds[1, 1]) / 2,
                    local.bounds[1, 2] + 0.018,
                ]
            )
            point = np.asarray(obj.anchors.get("label", point), dtype=float)
            label = scene.add_label(
                "/annotations/" + obj.object_id,
                obj.object_id,
                visible=False,
            ...
            self.frames[obj.object_id] = parent
            self.labels[obj.object_id] = label
            self.handles.extend([parent, child, label])
        self.leaders = scene.add_line_segments(
            "/annotations/leaders",
    ...
    def update_annotations(self):
        ...
        key = (
            tuple(cam.position),
            tuple(cam.look_at),
            tuple(cam.up_direction),
            cam.aspect,
            cam.fov,
            tuple((oid, pose.position_m, pose.wxyz) for oid, pose in sorted(self.current_poses.items())),
        )
        if key == self.annotation_key:
            return
        anchors = {
            oid: transform_points(self.current_poses[oid], np.asarray([point]))[0]
            for oid, point in self.label_anchors.items()
        }
        layout = border_annotations(
            anchors, cam.position, cam.look_at, np.rad2deg(cam.fov), cam.aspect, cam.up_direction
        )
        for label in self.labels.values():
            label.visible = False
        for item in layout:
            ...
            label.visible = True
        self.leaders.points = np.asarray([item.leader for item in layout], dtype=np.float32).reshape(-1, 2, 3)
    ...
    def select_view(self, mode, object_id=None):
        ...
        cam.far = 100.0
        if mode == "inspection":
            pos, look, up = inspect_view(
                self.objects[self.selected_id],
                self.current_poses[self.selected_id],
                self.spec.render_hints.fov_y_deg,
                cam.aspect,
            )
        elif mode == "detail":
            ...
            cue = (
                next((c for c in self.packet.cues if c.target_id == self.selected_id), None)
                if self.packet
                else None
            )
            direction = np.asarray(self.spec.camera_position_m) - self.spec.camera_look_at_m
            pos, look, up = detail_view(
                obj,
                self.current_poses[self.selected_id],
                asset.bounds,
                direction,
                cue.action if cue else None,
                self.spec.render_hints.fov_y_deg,
                cam.aspect,
            ...
            pos, look = fit(
                ...
                self.spec.render_hints.fov_y_deg,
                cam.aspect,
                annotation_margin(list(self.objects), cam.aspect, self.spec.render_hints.viewport_margin),
            )
            up = (0.0, 0.0, 1.0)
        if self.guidance_enabled:
            pos, look = selected_cue_view(
                self.spec,
                self.packet,
                self.current_poses,
                self.clock.elapsed,
    ...
    def focus_distance(self, object_id):
        ...
        inspection_focus = self.view_mode == "inspection" or (
            self.packet is not None
            and any(cue.target_id == object_id and cue.action == "inspect_back" for cue in self.packet.cues)
        )
    ...
    def tick(self, dt, now, focus_m, brightness, enabled, running):
        ...
        if self.packet is None:
            return
        cam = self.client.camera
        moving = running and any(
            c.task_role == "current"
            and c.action in ("rotate", "insert", "assemble")
            and self.clock.elapsed.get(c.task_id, 0.0) < c.interaction.duration_s
            for c in self.packet.cues
        )
        for cue in self.packet.cues:
            self.clock.advance(cue.task_id, dt, running and cue.task_role == "current")
        refresh = (moving and now - self.last_response >= 0.15) or self.response_key is None
        key = (
            round(focus_m, 4),
            round(brightness, 3),
            enabled,
            tuple(np.round(cam.position, 4)),
        ...
        with self.client.atomic():
            for cue in self.packet.cues:
                ...
                if refresh or cue.task_id not in self.cues:
                    centers, covariances, rgbs, opacities = splat_arrays(
                        self.spec, cue, pose, cam.position, cam.look_at, focus_m, brightness
                    )
                    if cue.task_id not in self.cues:
                        self.cues[cue.task_id] = self.client.scene.add_gaussian_splats(
                            f"/guidance/{cue.task_id}",
                            centers=centers,
                ...
                handle.visible = enabled and (
                    self.view_mode != "inspection" or cue.target_id == self.selected_id
                )
        if refresh:
