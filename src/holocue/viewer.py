...
class Operator:
    def __init__(self, client: viser.ClientHandle, base: str):
        ...
        with tabs.add_tab("任务"):
            self.scene_choice = gui.add_dropdown(
                "场景", options=list(self.by_title), initial_value=initial["title"], disabled=True
            )
            self.command = gui.add_text("任务指令", initial_value=initial["initial_instruction"])
            self.actions = gui.add_button_group(
                "任务操作", ["发送任务", "暂停动作", "确认完成", "恢复任务"], visible=False
            )
            self.playback_speed = gui.add_number(
                "动作播放速度", min=0.1, max=2.0, step=0.05, initial_value=1.0
            )
            self.message = gui.add_markdown("选择任务并提交指令")
            self.task_list = gui.add_markdown("任务队列为空")
            self.suspended_list = gui.add_markdown("")
            self.session_field = gui.add_text(
                "会话标识", initial_value=os.environ.get("HOLOCUE_SESSION_ID", "")
            )
            self.open_button = gui.add_button("打开已有会话")
        with tabs.add_tab("观察"):
            self.follow = gui.add_checkbox("跟随当前步骤", initial_value=True)
            self.target = gui.add_dropdown(
                "观察对象", options=["等待场景"], initial_value="等待场景", disabled=True
            )
        ...
        with tabs.add_tab("显示响应"):
            self.focus = gui.add_slider("焦点距离 米", min=0.05, max=20.0, step=0.005, initial_value=1.0)
            self.brightness = gui.add_slider("提示亮度", min=0.0, max=4.0, step=0.05, initial_value=1.0)
            self.focus_number = gui.add_number(
                "焦点数值 米", min=0.05, max=20.0, step=0.005, initial_value=1.0
            )
            self.brightness_number = gui.add_number(
                "亮度数值", min=0.0, max=4.0, step=0.05, initial_value=1.0
            )
            ...
            self.response_note = gui.add_markdown(
                ...
                + str(profile().visual_magnification)
                + "。光学标定由物理后端提供。"
            )
        ...
        def operation(event):
            ...
            if label == "发送任务":
                ...
                self.launch(
                    self.control, {"暂停动作": "pause", "确认完成": "complete", "恢复任务": "resume"}[label]
                )
        ...
        def select_view(event):
            mode = {"工作区域": "workspace", "目标特写": "detail", "结构检查": "inspection"}[
                event.target.value
            ]
            with self.lock:
        ...
        def resized(_):
            with self.lock:
                if (
                    self.renderer
                    and self.loaded
                    and getattr(self, "camera_aspect", None) != client.camera.aspect
                ):
    ...
    def record(self, kind, data):
        with self.log_file.open("a", encoding="utf-8") as file:
            file.write(
                json.dumps(
                    {"time": time.time(), "kind": kind, "session_id": self.sid, **data}, ensure_ascii=False
                )
                + "\n"
            )
    ...
    def refresh_object_summary(self):
        if self.renderer is not None and self.state is not None:
            self.inspect_text.content = object_summary(
                self.renderer.spec, self.state, self.target.value
            ).markdown
    ...
    def load_scene_session(self, scene_id, sid):
        ...
        with self.lock:
            ...
            self.session_field.value = self.sid
            ...
            self.packet = None
            self.state = Session.model_validate(state)
            self.revision = -1
            self.epoch = -1
            self.gate = VersionGate()
            self.failed = False
    ...
    def submit(self, text):
        ...
        receipt = self.request(
            ...
            json={"text": text, "request_id": uuid.uuid4().hex, "expected_revision": state["revision"]},
        )
        with self.lock:
            if generation == self.generation:
    ...
    def accept(self, data, sid, generation):
        state = data["state"]
        packet = DisplayPacket.model_validate(data["display"])
        ...
        with self.lock:
            ...
            if packet.revision == self.revision and packet.epoch == self.epoch:
                return
            old_current = (
                next((c.task_id for c in self.packet.cues if c.task_role == "current"), None)
                if self.packet
                else None
            )
            ...
            ) or ("临时任务已完成，原计划等待手动恢复。" if state["suspended"] else "当前没有待执行步骤。")
            if state["suspended"]:
                saved = "\n\n".join(
                    f"{i + 1}. {task['semantic']['target_id']}　{task['semantic']['instruction']}"
                ...
                self.suspended_list.content = (
                    f"**已挂起 {len(state['suspended'])} 组计划，最近一组如下**\n\n"
                    + saved
            ...
            self.response_values.content = "\n\n".join(
                f"{c.target_id}　{ROLE_TEXT[c.task_role]}　N {c.n_gaussians}　σ {c.sigma_value:g}"
            ...
            if current and self.follow.value:
                ...
                if old_current != current.task_id or self.renderer.selected_id != current.target_id:
                    mode = (
                        "inspection"
                        if current.action == "inspect_back"
                        else "detail"
                        if current.action == "rotate"
                        else "workspace"
                    )
            ...
            self.record(
                "snapshot",
                {
                    ...
                    "tasks": [c.task_id for c in packet.cues],
                },
            )
        ...
        while not self.stop.wait(1 / 30):
            ...
            with self.lock:
                ...
                running = (
                    not self.pending
                    and not self.failed
                    and now - self.last_snapshot < 0.8
                    and self.packet is not None
                    and self.packet.execution == "running"
                    and self.gate.current(self.packet.revision, self.packet.epoch)
                )
                self.renderer.tick(
                    dt * self.playback_speed.value,
                    now,
                    self.focus.value,
                ...
                if now - self.last_view_export >= 0.2:
                    cam = self.client.camera
                    atomic_json(
                        self.log_dir / f"client_{self.client.client_id}_view.json",
                        {
                            "session_id": self.sid,
                            "scene_id": self.renderer.spec.scene_id,
                            "revision": self.revision,
                            "epoch": self.epoch,
                            "execution": self.packet.execution if self.packet else "loading",
                            "clock_advancing": running,
                            "generated_at": time.time(),
                            "position_m": list(cam.position),
                            "look_at_m": list(cam.look_at),
                            "focus_m": self.focus.value,
                            "up_direction": list(cam.up_direction),
                            "fov_rad": float(cam.fov),
                            "aspect": float(cam.aspect),
                            "brightness": self.brightness.value,
                            "enabled": self.enabled.value,
                            "playback_speed": self.playback_speed.value,
                            "view_mode": self.renderer.view_mode,
                            "selected_id": self.renderer.selected_id,
