...
def decision_schema(session:Session,scene:SceneSpec)->dict:
    ...
    base['properties']={key:base['properties'][key]
                        for key in ('operation','cues','assistant_message')}
    for name,roles in (('CurrentCue',['current']),('NextCue',['next']),('BackgroundCue',['background'])):
        variants=[]
        for action in ('point','rotate','inspect_back','insert','assemble','wait'):
            ...
            fields['angle_deg']=({'type':'number','minimum':-360,'maximum':360}
                                 if action=='rotate' else {'type':'null'})
            fields['reference_id']=({'type':'string','minLength':1}
                                    if action in ('insert','assemble') else {'type':'null'})
            kinds={'rotate':['ring_arrow'],'insert':['ghost_motion'],'assemble':['ghost_motion'],
                   'inspect_back':['highlight'],'point':['highlight','straight_arrow','label'],'wait':['label']}
            fields['cue_type']={'type':'string','enum':kinds[action]}
            cue['required']=list(fields)
            variants.append(cue)
        definitions[name]={'anyOf':variants}
    definitions['FollowingCue']={'anyOf':[
        *definitions.pop('NextCue')['anyOf'],*definitions.pop('BackgroundCue')['anyOf']]}
    allowed=permitted_operations(session)
    plan=deepcopy(base)
    plan['properties']['operation']['enum']=[value for value in allowed if value in ('replace','interrupt')]
    plan['properties']['assistant_message']={'type':'null'}
    plan['properties']['cues']={'type':'array','minItems':1,'maxItems':8,
        'prefixItems':[{'$ref':'#/$defs/CurrentCue'}],
        'items':{'$ref':'#/$defs/FollowingCue'}}
    plan['required']=list(plan['properties'])
    control=deepcopy(base)
    control['properties']['operation']['enum']=[value for value in allowed if value not in ('replace','interrupt','clarify')]
...
class OpenAICompatiblePlanner:
    ...
    def request_body(self,s:Session,scene:SceneSpec,msg:UserMessage):
        ...
        scene_view=[{'object_id':o.object_id,'label':o.label,'description':o.description,
                     'capabilities':o.capabilities,'rotation_axis_local':o.interaction.rotation_axis_local} for o in scene.objects]
        context={'permitted_operations':permitted_operations(s),
                 'task_state':('temporary_task_active' if s.queue and s.suspended else
                               'active_plan' if s.queue else 'awaiting_resume' if s.suspended else 'empty'),
                 'scene_id':scene.scene_id,'setting':scene.task_contract.setting,
                 'objects':scene_view,'queue':[t.model_dump() for t in s.queue],
        ...
        body={'model':self.model,'messages':[{'role':'system','content':self.prompt},{'role':'user','content':content}],
              ...
              'response_format':{'type':'json_schema','json_schema':{'name':'semantic_decision','strict':True,'schema':decision_schema(s,scene)}}}
        return body
