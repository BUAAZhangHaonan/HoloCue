...
class Interaction(Strict):
    rotation_axis_local: tuple[float, float, float] = (0.0, 0.0, 1.0)
    pivot_local_m: tuple[float, float, float] = (0.0, 0.0, 0.0)
    mating_pose: Pose = Field(default_factory=Pose)
    departure_axis_local: tuple[float, float, float] = (0.0, 0.0, 1.0)
    departure_distance_m: float = Field(default=0.08, gt=0)
    approach_axis_local: tuple[float, float, float] = (0.0, 0.0, 1.0)
    approach_distance_m: float = Field(default=0.12, gt=0)
    transit_height_m: float = Field(default=0.15, ge=0)
    inspect_point_local_m: tuple[float, float, float] = (0.0, 0.0, 0.0)
    inspect_normal_local: tuple[float, float, float] = (0.0, 1.0, 0.0)
    inspect_extent_m: tuple[float, float] | None = None
    inspect_cue_clearance_m: float = Field(default=0.002, ge=0)
    detail_direction_local: tuple[float, float, float] | None = None
    cue_offset_local_m: tuple[float, float, float] = (0.0, 0.0, 0.04)
    duration_s: float = Field(default=3.0, gt=0)

    @field_validator(
        "rotation_axis_local",
        "departure_axis_local",
        "approach_axis_local",
        "inspect_normal_local",
        "detail_direction_local",
    )
    @classmethod
...
class SceneObject(Strict):
    ...
    recipe: str = ""
    size_m: tuple[float, float, float] = (0.10, 0.10, 0.10)
...
class SceneProp(Strict):
    ...
    scale_m: tuple[float, float, float] = (1.0, 1.0, 1.0)


class RenderHints(Strict):
    ortho_scale_m: float = Field(default=0.91, gt=0)
    grid_extent_m: float = Field(default=1.0, gt=0)
    cue_scale: float = Field(default=1.0, gt=0)
    label_offset_m: float = Field(default=0.12, ge=0)
    fit_camera: bool = True
    fill_lights: list[tuple[float, float, float, float, float]] = Field(default_factory=list)
    fov_y_deg: float = Field(default=42.0, ge=20, le=80)
    viewport_margin: float = Field(default=0.14, ge=0.05, le=0.35)
    ...
    environment_wxyz: tuple[float, float, float, float] = (1.0, 0.0, 0.0, 0.0)
...
class SceneSpec(Strict):
    ...
    camera_position_m: tuple[float, float, float] = (0.8, -0.85, 0.72)
    camera_look_at_m: tuple[float, float, float] = (0.0, 0.12, 0.08)
    ...
    def unique_ids(self):
        ...
        if len(ids) != len(set(ids)):
            raise ValueError("duplicate scene object IDs")
        prop_ids = [p.prop_id for p in self.environment]
        if len(prop_ids) != len(set(prop_ids)):
            raise ValueError("duplicate environment prop IDs")
        if self.task_contract:
            ...
            if not references.issubset(ids):
                raise ValueError("task contract references an unknown object")
        return self
    ...
    def decision_shape(self):
        ...
        if new_plan:
            ...
            for cue in self.cues:
                if cue.task_role == "background":
                    seen_background = True
                elif seen_background:
                    raise ValueError("background cues must follow all actionable steps")
        elif self.cues:
...
class DisplayCue(Strict):
    ...
    reference_id: str | None = None
    interaction: Interaction = Field(default_factory=Interaction)
    cue_extent_m: float = Field(default=0.10, gt=0)
