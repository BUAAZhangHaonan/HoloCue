...
def annotation_metrics(identifiers, aspect: float):
    ...
    height = min(0.030, 0.72 * aspect / characters)
...
def annotation_margin(identifiers, aspect: float, base: float) -> float:
    ...
    return max(base, width + 0.065)


def border_annotations(
    anchors: dict[str, np.ndarray], position, look_at, fov_y_deg: float, aspect: float, up=(0.0, 0.0, 1.0)
) -> list[Annotation]:
    ...
    projected = np.column_stack(
        (local @ right / (depths * tangent * aspect), local @ vertical / (depths * tangent))
    )
    in_frame = np.all(np.abs(projected) < 1.0, axis=1)
    ...
    if not ids:
        return []
    rows = (len(ids) + 1) // 2
    ys = np.linspace(0.80, -0.80, rows) if rows > 1 else np.array([0.0])
    x = 0.96 - width / 2
    ...
    for side in (-1, 1):
        ...
        fitted = np.clip(fitted, -0.86, 0.86 - gap * max(len(ordered) - 1, 0))
    ...
    for i, (sx, sy) in zip(row, assigned):
        ...
        results.append(
            Annotation(
                ...
                tuple(end),
                height * half_height,
                (sx - width / 2, sy - height * 0.9, sx + width / 2, sy + height * 0.9),
