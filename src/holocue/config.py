...
def load_scene(scene_id: str) -> SceneSpec:
    ...
    result = SceneSpec.model_validate(
        json.loads(p.read_text(encoding="utf-8"), object_pairs_hook=unique_keys)
    )
...
def list_scenes() -> list[dict]:
    return [
        {"scene_id": s.scene_id, "title": s.title, "initial_instruction": s.initial_instruction}
        for s in (
            load_scene(p.parent.name)
