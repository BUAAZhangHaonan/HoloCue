...
def basis(position, look_at, up=(0.,0.,1.)):
...
def fit(points: np.ndarray, direction, fov_y_deg=42., aspect=1.6, margin=.14, up=(0.,0.,1.)):
    ...
    distance = max(float(np.max(np.abs(y)/ty-z)),
                   float(np.max(np.abs(x)/(ty*aspect)-z)), .1)
...
def project(points, position, look_at, fov_y_deg=42., aspect=1.6, up=(0.,0.,1.)):
...
def focus_point(obj: SceneObject, pose: Pose, inspection: bool):
    ...
    if inspection:
        local = np.asarray(local)+np.asarray(obj.interaction.inspect_normal_local)*obj.interaction.inspect_cue_clearance_m
...
def retreat_to_fit(points, position, look_at, fov_y_deg, aspect, up):
    ...
    retreat = max(0., float(np.max(np.abs(relative@right)/(tangent*aspect)-depth)),
...
def detail_view(obj: SceneObject,pose: Pose,bounds,default_direction,action=None,
                fov_y_deg=42.,aspect=1.6,margin=.14):
    ...
    up=(0.,1.,0.) if abs(direction[2])>.95 else (0.,0.,1.)
...
def inspection_points(obj: SceneObject, pose: Pose):
    ...
    return point+np.asarray([-right*width/2-vertical*height/2,right*width/2-vertical*height/2,
                             right*width/2+vertical*height/2,-right*width/2+vertical*height/2])


def inspect_view(obj: SceneObject, pose: Pose, fov_y_deg=42., aspect=1.6, margin=.16):
    point,normal,up,_,_,width,height=inspection_region(obj,pose)
    tangent=np.tan(np.deg2rad(fov_y_deg)/2)*(1-margin)
    distance=max(height/(2*tangent),width/(2*tangent*aspect),.05)
