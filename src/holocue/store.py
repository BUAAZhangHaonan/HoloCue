...
class Store:
    ...
    def create(self, scene_id, backend_mode, scene_fingerprint=""):
        ...
        with self.connection() as c:
            self._save(c, s)
            self._event(c, s.session_id, "created", s.model_dump())
        return s

    def get(self, sid):
        with self.connection() as c:
            return self._read(c, sid)

    def begin(self, sid, msg: UserMessage):
        fingerprint = hashlib.sha256(
            json.dumps(
                {"text": msg.text, "image": msg.image_base64, "mime": msg.image_mime}, sort_keys=True
            ).encode()
        ).hexdigest()
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE")
            old = c.execute(
                "SELECT * FROM jobs WHERE session_id=? AND request_id=?", (sid, msg.request_id)
            ).fetchone()
            if old:
            ...
            s = self._read(c, sid)
            if s.revision != msg.expected_revision:
            ...
            job = {
                ...
                "epoch": s.epoch,
                "status": "planning",
                "data": json.dumps({"started_at": time.time(), "text": msg.text}, ensure_ascii=False),
            }
    ...
    def commit(self, job_id, d: Decision, scene: SceneSpec, trace: dict):
        with self.connection() as c:
            ...
            if r["status"] != "planning" or s.epoch != r["epoch"]:
                return None
            new = apply_decision(s, d, scene)
            self._save(c, new)
            info = json.loads(r["data"])
            info.update(trace)
            info.update({"finished_at": time.time(), "decision": d.model_dump(), "revision": new.revision})
            c.execute(
                "UPDATE jobs SET status='done',data=? WHERE id=?",
                (json.dumps(info, ensure_ascii=False), job_id),
            )
    ...
    def fail(self, job_id, error: str, trace: dict | None = None):
        with self.connection() as c:
            ...
            self._save(c, s)
            info = json.loads(r["data"])
            info.update(trace or {})
            info.update({"error": error, "finished_at": time.time()})
