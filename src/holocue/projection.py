...
def allocate(weights: list[int], budget: int) -> list[int]:
    if budget < 0 or any(w < 0 for w in weights):
        raise ValueError("negative budget/weight")
    total = sum(weights)
    if total == 0:
...
def packet(s: Session, scene: SceneSpec, policy: dict) -> DisplayPacket:
    ...
    for task in s.completed:
        ...
        if semantic.action == "rotate":
            poses[obj.object_id] = rotate_about(
                poses[obj.object_id],
                obj.interaction.rotation_axis_local,
                obj.interaction.pivot_local_m,
                semantic.angle_deg,
            )
    ...
    for t, ni in zip(tasks, n):
        ...
        cues.append(
            DisplayCue(
                ...
                n_gaussians=ni,
                sigma_value=prof["sigma"],
                sigma_units=calibration["sigma_units"],
                sigma_profile=prof["name"],
                pose=poses[obj.object_id],
                goal_pose=goal,
                angle_deg=c.angle_deg,
                reference_id=c.reference_id,
                interaction=obj.interaction,
                cue_extent_m=max(obj.size_m),
            )
        )
