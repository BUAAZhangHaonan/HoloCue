...
def basis_from_normal(normal):
    ...
    n/=np.linalg.norm(n)
    helper=np.array([0.,0.,1.]) if abs(n[2]) < .9 else np.array([0.,1.,0.])
    x=np.cross(helper,n);x/=np.linalg.norm(x)
    y=np.cross(n,x)
...
def _local_cue_pool(scene_id: str,target_id: str,kind: str,fingerprint: str,count: int) -> np.ndarray:
    ...
    u=rng.random(count)
    radius=max(min(obj.size_m)*.72,.014)
    if kind=='ring_arrow':
        angle=(.15+1.65*u)*np.pi
        points=np.column_stack([radius*np.cos(angle),radius*np.sin(angle),np.zeros(count)])
        head=u>.88
        a=1.80*np.pi
        tip=np.array([np.cos(a),np.sin(a)])*radius
        back=tip+np.array([np.sin(a),-np.cos(a)])*radius*.42
        side=np.array([np.cos(a),np.sin(a)])*radius*.22
        v=rng.random(head.sum());w=rng.random(head.sum())
        points[head,:2]=tip+(back-tip)*np.sqrt(v)[:,None]+side*(2*w-1)[:,None]*np.sqrt(v)[:,None]
...
def splat_arrays(scene: SceneSpec,cue: DisplayCue,pose: Pose,camera_position,look_at,
                 focus_m: float,brightness: float=1.,calibration: ResponseProfile | None=None):
    ...
    if cue.action=='inspect_back':
        ...
        centers+=np.asarray(cue.interaction.inspect_normal_local,dtype=np.float32)*cue.interaction.inspect_cue_clearance_m
    ...
    optical_depth=cal.per_splat_optical_depth*cal.reference_count/max(cue.n_gaussians,1)*peaks*brightness
    opacity=-np.expm1(-optical_depth)
    rgb=np.tile(np.array([[.12,.80,.64]],np.float32),(cue.n_gaussians,1))
    return centers,covariance.astype(np.float32),rgb,opacity[:,None].astype(np.float32)


def camera_aligned_rgba(centers,covariances,opacities,rgbs,world_pose,position,look_at,
                        size=(640,400),fov_y_deg=42.):
    ...
    px=points@right/np.maximum(depth,.001)*focal+width/2
    py=height/2-points@up/np.maximum(depth,.001)*focal
    std=np.sqrt(covariances[:,0,0])/np.maximum(depth,.001)*focal
