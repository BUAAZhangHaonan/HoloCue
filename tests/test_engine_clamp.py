...
def fingerprint(mesh):
    result = {
        key: hashlib.sha256(np.asarray(getattr(mesh, key), dtype=dtype).tobytes()).hexdigest()
    ...
    result["material"] = {
        "name": material.name,
        "baseColorFactor": np.asarray(material.baseColorFactor).tolist(),
        ...
        "texture_sha256": None
        if texture is None
        else hashlib.sha256(np.asarray(texture).tobytes()).hexdigest(),
    }
    result["uv"] = (
        None
        if mesh.visual.uv is None
        else hashlib.sha256(np.asarray(mesh.visual.uv, dtype="<f8").tobytes()).hexdigest()
    )
    return result
...
def field(mesh):
    copy = mesh.copy()
    copy.process(validate=True)
    result = vtk.vtkImplicitPolyDataDistance()
    result.SetInput(polydata(copy))
    return result
...
def roof(y):
    y = np.asarray(y)
    result = np.full(y.shape, 1.122)
    left = (y > -0.166) & (y < -0.116)
    t = (y[left] + 0.166) / 0.05
    result[left] = 1.122 - 0.035 * (3 * t * t - 2 * t * t * t)
    result[(y >= -0.116) & (y <= -0.084)] = 1.087
    right = (y > -0.084) & (y < -0.034)
    t = (y[right] + 0.084) / 0.05
    result[right] = 1.087 + 0.035 * (3 * t * t - 2 * t * t * t)
    return result
...
def line_hits(mesh, start, end):
    ...
    e1 = tri[:, 1] - tri[:, 0]
    e2 = tri[:, 2] - tri[:, 0]
    cross = np.cross(delta, e2)
    det = np.einsum("ij,ij->i", e1, cross)
    mask = np.abs(det) > 1e-15
    tri = tri[mask]
    e1 = e1[mask]
    e2 = e2[mask]
    cross = cross[mask]
    det = det[mask]
    offset = start - tri[:, 0]
    u = np.einsum("ij,ij->i", offset, cross) / det
    q = np.cross(offset, e1)
    v = q @ delta / det
    t = np.einsum("ij,ij->i", e2, q) / det
...
def engine(tmp_path_factory):
    ...
    for obj in [*spec.objects, *spec.environment]:
        ...
        generated.update(meshes(assembly.scene, obj.pose))
        roundtrip.update(meshes(loaded, obj.pose))
        local[identifier] = meshes(loaded)
    old = meshes(
        load_asset(str(fixtures / "engine_hose_before.glb"), spec.asset_axes), spec.environment[0].pose
    )
    old.update(
        meshes(load_asset(str(fixtures / "engine_clamp_before.glb"), spec.asset_axes), spec.objects[0].pose)
    )
    hose_solids = engine_hose_solids()
    hose_union = hose_solids[0]
    for solid in hose_solids[1:]:
        hose_union = hose_union.union(solid)
...
def test_only_authorized_nodes_change_and_materials_stay(engine):
    changed = {
        "workstation": {"0052_tube", "0053_tube", "0054_rounded_part", "0055_clamp_band"},
...
def test_material_regions_are_closed_and_physical_hose_is_connected(engine, version):
    ...
    for suffix in ("0052_tube", "0053_tube", "0054_rounded_part", "0055_clamp_band"):
        mesh = part(engine[version], suffix).copy()
...
def test_head_slot_and_shaft_preserve_real_dimensions_and_connections(engine, version):
    ...
    for suffix in ("0010_hex_head", "0011_driver_slot"):
        new = part(actual, suffix)
        old = part(engine["old"], suffix)
        assert np.array_equal(new.faces, old.faces)
        assert np.allclose(new.vertices - [0.019, 0, 0], old.vertices, atol=8e-8, rtol=0)
    ...
    shaft = part(actual, "0000_threaded_shank")
    old = part(engine["old"], "0000_threaded_shank")
    assert abs(shaft.bounds[0, 0] - old.bounds[0, 0]) < 8e-8
    assert abs(shaft.bounds[1, 0] - old.bounds[1, 0] - 0.019) < 8e-8
    assert np.allclose(shaft.bounds[:, 1:], old.bounds[:, 1:], atol=8e-8, rtol=0)
    assert shaft.bounds[1, 0] > part(actual, "0010_hex_head").bounds[0, 0] + 0.0058
    # Interior joint points certify that the long shaft reaches the original head.
    witness = np.array([[-0.270, -0.100, 1.100]])
    assert distances(field(shaft), witness)[0] < -0.001
    assert distances(field(part(actual, "0010_hex_head")), witness)[0] < -0.001


@pytest.mark.parametrize("version", ["generated", "roundtrip"])
def test_actual_metal_blind_bore_ring_grooves_and_band_union(engine, version):
    mesh = part(engine[version], "0055_clamp_band")
    seat_field = field(mesh)
    axial = line_hits(mesh, [-0.34, -0.1, 1.1], [-0.25, -0.1, 1.1])[:, 0]
    assert np.allclose(axial, [-0.32421, -0.32021], atol=2e-6, rtol=0), axial
    sections = []
    for xx in np.linspace(-0.3188, -0.29154, 9):
        hits = line_hits(mesh, [xx, -0.1, 1.075], [xx, -0.1, 1.13])[:, 2]
        assert len(hits) == 4, (xx, hits)
        assert abs(hits[1] - 1.0934) < 3e-5 and abs(hits[2] - 1.1066) < 3e-5, (xx, hits)
        assert hits[3] - hits[2] > 0.00435, (xx, hits)
        sections.append({"x_m": float(xx), "actual_z_intersections_m": hits.tolist()})
    bore = line_hits(mesh, [-0.28, -0.1, 1.075], [-0.28, -0.1, 1.13])[:, 2]
    assert np.allclose(bore, [1.089, 1.0948, 1.1052, 1.111], atol=3e-5, rtol=0), bore
    witness = np.array([[-0.285, -0.110, 1.1]])
    assert distances(field(part(engine["old"], "0055_clamp_band")), witness)[0] < -0.00149
    assert distances(seat_field, witness)[0] < -0.0009
    ...
    old = part(engine["old"], "0055_clamp_band")
    points, _ = trimesh.sample.sample_surface(old, 1536, seed=4701)
    assert distances(seat_field, points).max() < 2e-6
    assert distances(seat_field, np.array([[-0.306, -0.1, 1.088]]))[0] < -0.0005
    (engine["directory"] / f"metal_sections_{version}.json").write_text(
        json.dumps(
            {"ring_grooves": sections, "blind_bore_x_m": axial.tolist(), "plain_bore_z_m": bore.tolist()},
            indent=2,
        )
    )
...
def test_real_hose_sections_have_flattened_lumen_and_ten_mm_roof(engine, version):
    ...
    records = []
    for yy in (-0.116, -0.114, -0.1, -0.086, -0.084):
        xx = -0.32 + (yy + 0.12) * 0.05 / 0.18
...
def test_unchanged_hose_exterior_and_endpoints(engine, version):
    records = []
    old_fields = [
        field(part(engine["old"], suffix)) for suffix in ("0052_tube", "0053_tube", "0054_rounded_part")
    ]
    for suffix in ("0052_tube", "0053_tube", "0054_rounded_part"):
        old = part(engine["old"], suffix)
        new = part(engine[version], suffix)
        ...
        keep = points[:, 2] < roof(points[:, 1]) - 0.0001
        ...
        if suffix != "0054_rounded_part":
            segment = np.array([[-0.46, -0.25, 1.04], [-0.32, -0.12, 1.06], [-0.27, 0.06, 1.03]])
            index = 0 if suffix == "0052_tube" else 1
            axis = segment[index + 1] - segment[index]
            axis /= np.linalg.norm(axis)
            keep &= np.abs(old.face_normals[faces] @ axis) < 0.01
            endpoint = segment[0] if index == 0 else segment[2]
            near_end = np.abs((old.vertices - endpoint) @ axis) < 1e-7
            rim = old.vertices[near_end]
            rim = rim[np.linalg.norm(rim - endpoint, axis=1) > 0.05]
            assert len(rim) > 0
        ...
        records.append(
            {"node": suffix, "surface_samples": int(keep.sum()), "max_exterior_error_m": float(error.max())}
        )
...
def test_actual_flow_sections_and_wall_measurements(engine, version):
    hose = [
        part(engine[version], suffix).copy() for suffix in ("0052_tube", "0053_tube", "0054_rounded_part")
    ]
    for mesh in hose:
        mesh.process(validate=True)
    trees = []
    for mesh in hose:
        tree = vtk.vtkOBBTree()
        tree.SetDataSet(polydata(mesh))
        tree.BuildLocator()
        trees.append(tree)
    hose_fields = [field(mesh) for mesh in hose]
    boundary_field = field(engine["hose_union"])
    path = np.array([[-0.46, -0.25, 1.04], [-0.32, -0.12, 1.06], [-0.27, 0.06, 1.03]])
    rows = []
    for segment, (aa, bb) in enumerate(zip(path, path[1:])):
        ...
        for t in np.linspace(0.001, 0.999, 25):
            ...
            for theta in np.linspace(0, 2 * np.pi, 180, endpoint=False):
                ...
                end = center + 0.10 * direction
                hits = []
                for tree in trees:
                    points = vtk.vtkPoints()
                    ids = vtk.vtkIdList()
                    tree.IntersectWithLine(center, end, points, ids)
                    hits.extend(
                        np.linalg.norm(np.array(points.GetPoint(i)) - center)
            ...
            assert radii.min() > 0.0168, (segment, t, radii.min())
            assert area > 0.00460, (segment, t, area)
            rows.append(
                {
                    "segment": segment,
                    "t": float(t),
                    "center_m": center.tolist(),
                    "actual_normal_flow_area_m2": area,
                    "actual_inner_radii_m": radii.tolist(),
                }
            )
    ...
    for mesh in hose:
        ...
        for aa, bb in zip(path, path[1:]):
            axis = bb - aa
            t = np.clip((centers - aa) @ axis / (axis @ axis), 0, 1)
        ...
        is_roof = (np.abs(centers[:, 2] - roof(centers[:, 1])) < 0.0007) & (mesh.face_normals[:, 2] > 0.3)
        outer_roof.extend(mesh.triangles[is_roof])
        selected = exposed & (sign < -0.001) & (centers[:, 1] > -0.15) & (centers[:, 1] < -0.05)
        inner.extend(mesh.triangles[selected])
    outer_roof = np.asarray(outer_roof)
    inner = np.asarray(inner)
    assert len(outer_roof) > 10 and len(inner) > 100
    outer_mesh = trimesh.Trimesh(
        vertices=outer_roof.reshape(-1, 3), faces=np.arange(outer_roof.size // 3).reshape(-1, 3), process=True
    )
    ...
    original_union = load_asset(
        str(engine["fixtures"] / "engine_hose_outer_union_before.glb"), "project_z_up"
    ).to_geometry()
    ...
    evidence = {
        "normal_sections": rows,
        "actual_sampled_minimum_wall_m": float(gaps.min()),
        "inner_sample_count": len(samples),
        "outer_roof_triangle_count": len(outer_roof),
    ...
    assert gaps.min() > 0.006, float(gaps.min())
...
def test_original_full_ordered_clamp_and_plug_sweep(engine, monkeypatch):
        ...
        for frame, t in enumerate(np.linspace(0, source.interaction.duration_s, 41)):
            ...
            for oid, name, bounds, f in obstacles:
                ...
                if not selection.any():
                    continue
                d = distances(f, points[selection])
                penetration = d < -0.00075
                if penetration.any():
                    hits.append(
                        {
                            "frame": frame,
    ...
    assert all(
        any(
            c["part"].endswith("0055_clamp_band") and c["status"] == "included_closed_solid"
