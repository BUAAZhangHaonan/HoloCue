...
def test_controls_have_real_console_contact_and_clear_left_column(cnc,oid):
    ...
    assert combined.bounds[1,0] <= left[0,0]-.0049
    ...
    for mesh in objects[oid].values():
        ...
        supported=((triangles[:,:,0]>console[0,0]+.006)&
                   (triangles[:,:,0]<console[1,0]-.006)&
                   (triangles[:,:,2]>console[0,2]+.006)&
                   (triangles[:,:,2]<console[1,2]-.006)).all(axis=1)
        contact_area+=mesh.area_faces[on_plane & supported].sum()
    assert contact_area > .002
...
def test_magazine_mount_connects_hub_to_column_without_crossing_covers(cnc):
    ...
    assert disc[0,0] > spindle[1,0]+.008
    assert np.isclose(mount[0,1],hub[1,1],atol=1e-7)
    assert np.isclose(mount[1,1],column[0,1],atol=1e-7)
    assert np.all(mount[0,[0,2]]>column[0,[0,2]]+.006)
    assert np.all(mount[1,[0,2]]<column[1,[0,2]]-.006)
...
def test_workspace_rays_reach_each_target_before_environment(cnc):
    ...
    meshes=[];owners=[];ends=[];count=0;points=[]
    for oid,parts in [*objects.items(),('environment',environment)]:
        for mesh in parts.values():
            meshes.append(mesh);count+=len(mesh.faces);owners.append(oid);ends.append(count)
        if oid!='environment':
            points.extend(camera.corners(trimesh.util.concatenate(list(parts.values())).bounds))
    position,_=camera.fit(camera.workspace_points(spec,points),
        np.asarray(spec.camera_position_m)-spec.camera_look_at_m,aspect=1280/900)
    locator=vtk.vtkStaticCellLocator()
    locator.SetDataSet(polydata(trimesh.util.concatenate(meshes)));locator.BuildLocator()
    cell=vtk.vtkGenericCell()
    for obj in spec.objects:
        target=np.array(obj.pose.position_m)
        direction=target-position
        endpoint=target+direction/np.linalg.norm(direction)*.001
        t=vtk.mutable(0.);hit=[0.,0.,0.];pcoords=[0.,0.,0.]
        sub=vtk.mutable(0);cell_id=vtk.mutable(0)
        assert locator.IntersectWithLine(position,endpoint,1e-9,t,hit,pcoords,sub,cell_id,cell),obj.object_id
