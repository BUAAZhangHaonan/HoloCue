...
def rotation(target='B', role='current', depth='precise'):
    return CueSemantic(target_id=target, action='rotate', angle_deg=30,
        cue_type='ring_arrow', task_role=role, priority=2,
        depth_requirement=depth, instruction='逆时针转动三十度')
...
def temporary(target='C', action='inspect_back'):
    return Decision(operation='interrupt', assistant_message='执行临时任务', cues=[
        CueSemantic(target_id=target, action=action, cue_type='highlight',
            task_role='current', priority=2, depth_requirement='persistent', instruction='临时查看对象')])
...
def test_nullable_model_message_persists_structured_decision_and_keeps_old_history(tmp_path):
    ...
    context=json.loads(planner.request_body(reopened,scene,
        message(revision=reopened.revision))['messages'][1]['content'][0]['text'])
    assert context['history']==reopened.history
    paused=store.manual(reopened.session_id,reopened.revision,
        Decision(operation='pause',assistant_message='已暂停任务'),scene)
...
def test_unknown_target_and_unsupported_action_are_rejected():
    ...
    for target, error in [('UNKNOWN', 'Unknown object'), ('C', 'does not support rotate')]:
        with pytest.raises(DomainError, match=error):
            validate_decision(Decision(operation='replace', assistant_message='请求旋转', cues=[rotation(target)]), scene)
...
def test_new_plan_rejects_invisible_cues_without_changing_the_requested_weight(role):
    hidden=CueSemantic(target_id='A',action='point',cue_type='label',task_role=role,
                       priority=0,depth_requirement='neutral',instruction='零权重记录')
...
def test_positive_next_promotes_and_completes_through_real_store(tmp_path):
    store=Store(tmp_path/'positive.sqlite');scene=load_scene('control_panel')
...
def test_legacy_zero_next_completion_fails_atomically_and_remains_readable(tmp_path):
    store=Store(tmp_path/'legacy.sqlite');scene=load_scene('control_panel')
    legacy=store.create(scene.scene_id,'domain_test')
    hidden=rotation('A','next');hidden.priority=0
    legacy.queue=[Task(task_id='legacy-current',semantic=rotation()),
...
def test_background_cannot_precede_an_actionable_step():
    background=CueSemantic(target_id='C', action='point', cue_type='highlight',
        task_role='background', priority=1, depth_requirement='persistent', instruction='观察模块')
    with pytest.raises(ValidationError, match='background cues must follow'):
        Decision(operation='replace', assistant_message='错误顺序',
...
def test_count_and_depth_profile_are_independent():
    scene=load_scene('control_panel')
    decision=Decision(operation='replace', assistant_message='相同优先级的不同焦深需求',
...
def test_rotation_end_value_and_sign_follow_declared_axis(angle):
    ...
    state=apply_decision(session(),Decision(operation='replace',assistant_message='旋转角度测试',cues=[semantic]),scene)
    cue=packet(state,scene,load_policy()).cues[0]
    end=trajectory(cue,cue.interaction.duration_s)
    radians=np.deg2rad(angle)
    expected=np.array([[np.cos(radians),-np.sin(radians),0.],
                       [np.sin(radians), np.cos(radians),0.], [0.,0.,1.]])
    np.testing.assert_allclose(matrix(end)[:3,:3],expected,atol=1e-12)
    np.testing.assert_allclose(matrix(trajectory(cue,100.)),matrix(end),atol=1e-12)


def test_repeated_object_steps_keep_distinct_ordered_task_ids():
    scene=load_scene('control_panel')
    second=CueSemantic(target_id='B',action='point',cue_type='highlight',task_role='next',
        priority=2,depth_requirement='persistent',instruction='随后检查旋钮刻线')
    state=apply_decision(session(),Decision(operation='replace',assistant_message='先旋转再检查',
        cues=[rotation(),second]),scene)
...
def test_environment_ids_are_unique_and_not_planner_targets():
    ...
    with pytest.raises(DomainError,match='Unknown object'):
        validate_decision(Decision(operation='replace', assistant_message='无效目标', cues=[rotation(prop_id)]),scene)
...
def test_strict_request_disables_thinking_and_preserves_schema():
    ...
    assert body['response_format']['json_schema']['schema']==decision_schema(session(),load_scene('control_panel'))
...
def named_parts(scene, suffix):
    return [scene.geometry[scene.graph[node][1]] for node in scene.graph.nodes_geometry
...
def test_node3_has_open_fiber_ports_and_supported_cable_tails():
    ...
    for mesh in ports:
        ...
        assert radial.min()>.004
        assert radial.max()-radial.min()>.002
...
def test_pot3_has_closed_finite_wall_with_three_recessed_inner_bands():
    ...
    center=mesh.vertices[np.isclose(mesh.vertices[:,0],0.,atol=1e-9)]
    inner=center[center[:,1]>0]
    inner=inner[np.argsort(inner[:,2])]
    minima=np.flatnonzero((inner[1:-1,1]<inner[:-2,1]) & (inner[1:-1,1]<inner[2:,1]))+1
    assert len(minima)==3
    np.testing.assert_allclose(inner[minima,2]/obj.size_m[2],[-.22,0,.22],atol=.015)
    assert inner[:,1].max()-inner[:,1].min()>.002
    outer=center[center[:,1]<0]
    assert inner[:,1].min()-outer[:,1].max()>.010
...
def test_mirror_scale_and_support_are_below_optic_and_rear_label_is_textured():
    ...
    labels=[mesh for mesh in scene.geometry.values()
...
def test_added_cassette_locks_and_tool_insert_preserve_receiver_clearance():
    ...
    assert scene.extents[0]<receiver.size_m[0]-.012
    tool,scene=authored_object('cnc_toolchange','T09')
    inserts=named_parts(scene,'carbide_insert')
    assert inserts
    assert max(np.linalg.norm(mesh.vertices[:,:2],axis=1).max() for mesh in inserts)<.020
