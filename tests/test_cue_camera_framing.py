...
def cnc_packet(target, action):
    ...
    state = apply_decision(
        ...
        Decision(operation='replace', assistant_message='领域取景测试', cues=[
            CueSemantic(target_id=target, action=action,
                        cue_type='ring_arrow' if action == 'rotate' else 'highlight',
                        task_role='current', priority=3, depth_requirement='persistent',
                        instruction='检查相机取景', angle_deg=-90 if action == 'rotate' else None)]),
        scene)
...
def original_view(scene, display, target, mode, aspect):
    ...
    return camera.detail_view(
        obj, pose, mesh.bounds, np.asarray(scene.camera_position_m)-scene.camera_look_at_m,
        first.action if first else None, scene.render_hints.fov_y_deg, aspect,
...
@pytest.mark.parametrize('aspect', [1.276, .97])
@pytest.mark.parametrize('target,action,mode', [
...
def test_cnc_framing_keeps_real_cue_visible_without_changing_response(target, action, mode, aspect):
    ...
    cue = display.cues[0]
    elapsed = cue.interaction.duration_s*.4 if action == 'rotate' else 0.
    ...
    assert np.abs(before).max() > 1., 'This real CNC case must exercise previously clipped guidance'

    new_position, new_look = selected_cue_view(
        scene, display, display.object_poses, {cue.task_id: elapsed}, target, mode,
        position, look, up, aspect)
    points, arrays = rendered_quads(scene, display, cue, elapsed, mode, new_position, new_look, up)
    projected, depth = camera.project(points, new_position, new_look,
                                     scene.render_hints.fov_y_deg, aspect, up)
    assert depth.min() > 0
    ...
    if target == 'T03':
        ...
        region_ndc, _ = camera.project(region, new_position, new_look,
                                      scene.render_hints.fov_y_deg, aspect, up)
        ...
        full_tool_ndc, _ = camera.project(full_tool, new_position, new_look,
                                         scene.render_hints.fov_y_deg, aspect, up)
        assert np.abs(full_tool_ndc).max() > 1., 'The pull-stud view must not become a whole-tool overview'


@pytest.mark.parametrize('empty', [True, False])
def test_no_visible_cue_preserves_exact_camera(empty):
    scene, display = cnc_packet('T03', 'inspect_back')
    position, look, up = original_view(scene, display, 'T03', 'inspection', .97)
    if empty:
        ...
        brightness = 0.
        cue = display.cues[0]
        _, _, _, alpha = splat_arrays(scene, cue, cue.pose, position, look, .2, brightness)
        assert np.count_nonzero(alpha) == 0
    actual = selected_cue_view(scene, display, display.object_poses, {}, 'T03', 'inspection',
                               position, look, up, .97, brightness)
...
def test_repeated_object_keeps_rotate_as_first_visible_detail_action():
    ...
    semantic = [CueSemantic(
        target_id=step.target_id, action=step.action,
        cue_type='ring_arrow' if step.action == 'rotate' else 'highlight',
        task_role='current' if index == 0 else 'next', priority=3,
        depth_requirement=step.depth_requirement, instruction='领域有序步骤测试',
    ...
    state = apply_decision(
        ...
        Decision(operation='replace', assistant_message='领域测试', cues=semantic), scene)
    ...
    position, look, up = original_view(scene, display, 'M', 'detail', .97)
    new_position, new_look = selected_cue_view(scene, display, display.object_poses, {},
                                              'M', 'detail', position, look, up, .97)
    ...
    ndc, depth = camera.project(points, new_position, new_look,
                               scene.render_hints.fov_y_deg, .97, up)
    assert depth.min() > 0 and np.abs(ndc).max() <= 1.+1e-6
