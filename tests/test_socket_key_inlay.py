...
def polygon_clip(subject,boundary):
    ...
    for a,b in zip(boundary,np.roll(boundary,-1,axis=0)):
        ...
        if not before:break
        previous=before[-1];dp=cross(b-a,previous-a)
        for current in before:
            dc=cross(b-a,current-a)
            if (dc>=0)!=(dp>=0):output.append(previous+(current-previous)*dp/(dp-dc))
...
def horizontal_faces(mesh,z):
    return mesh.triangles[(np.abs(mesh.triangles[:,:,2]-z).max(axis=1)<1e-8)&(mesh.face_normals[:,2]>.999)]


def upward_hits(mesh,point):
    hits=[]
    for triangle in mesh.triangles:
        a,b,c=triangle;denominator=cross(b[:2]-a[:2],c[:2]-a[:2])
        if abs(denominator)<1e-16:continue
...
def test_socket_marker_has_a_real_roof_recess_and_keeps_the_opening(tmp_path,roundtrip):
    spec=load_scene('connector');obj=next(o for o in spec.objects if o.object_id=='S')
    assembly=make_object(obj);scene=assembly.scene
    if roundtrip:
        path=tmp_path/'S.glb';assembly.save(path)
    ...
    for node in scene.graph.nodes_geometry:
        transform,name=scene.graph[node];mesh=scene.geometry[name].copy();mesh.apply_transform(transform)
        parts[str(node)]=mesh
    roof=parts['S/0003_panel'];mark=parts['S/0009_key_channel_mark']
    x,y,z=obj.size_m;t=.005
    np.testing.assert_allclose(roof.bounds,[[-x/2+t,-y/2,z/2-t],[x/2-t,y/2,z/2]],atol=1e-8)
    np.testing.assert_allclose(mark.bounds,[[-.006,-y/2,.47*z-.0015],[.006,y/2,.47*z+.0015]],atol=1e-8)
    for mesh in (roof,mark):
        topology=mesh.copy();topology.merge_vertices(merge_tex=True,merge_norm=True)
    ...
    top_roof=horizontal_faces(roof,z/2);top_mark=horizontal_faces(mark,z/2)
    ...
    for xx in (-.004,0,.004):
        for yy in (-y*.35,0,y*.35):
            point=np.array([xx,yy]);rh=upward_hits(roof,point);mh=upward_hits(mark,point)
            assert rh and mh
            assert min(rh)==pytest.approx(z/2-t,abs=1e-8)
            assert max(rh)==pytest.approx(min(mh),abs=1e-8)
            assert min(mh)>min(rh)+.0019
    ...
    assert obj.anchors['insertion']==(0.,-.002,0.)
