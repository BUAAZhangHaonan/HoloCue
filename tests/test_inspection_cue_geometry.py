...
TARGETS=[(s['scene_id'],o.object_id) for s in list_scenes()
         for o in load_scene(s['scene_id']).objects if 'inspect_back' in o.capabilities]

@pytest.mark.parametrize('scene_id,target',TARGETS)
def test_inspection_ring_clears_mesh_and_point_pool(scene_id,target):
    spec=load_scene(scene_id);obj=next(o for o in spec.objects if o.object_id==target)
    decision=Decision(operation='replace',assistant_message='geometry test',cues=[
        CueSemantic(target_id=target,action='inspect_back',cue_type='highlight',
                    task_role='current',priority=1,depth_requirement='precise',instruction='inspect')])
    state=apply_decision(Session(session_id='geometry',scene_id=scene_id,backend_mode='domain_test'),decision,spec)
    ...
    centers,cov,rgb,alpha=splat_arrays(spec,cue,cue.pose,position,look,.2)
    ...
    point_centers,*_=splat_arrays(spec,point,point.pose,position,look,.2)
    ...
    a=tri[:,0,:2];b=tri[:,1,:2]-a;c=tri[:,2,:2]-a
    den=b[:,0]*c[:,1]-b[:,1]*c[:,0];valid=np.abs(den)>1e-12
