...
def test_current_message_occurs_once_and_history_is_preserved():
    ...
    session = Session(
        session_id="request",
        scene_id="control_panel",
        backend_mode="live",
        history=[{"role": "assistant", "content": "请选择任务。"}, {"role": "user", "content": text}],
    )
    body = OpenAICompatiblePlanner().request_body(
        session,
        load_scene("control_panel"),
...
@pytest.mark.parametrize(
    "has_queue,has_suspended,allowed",
    [
...
def test_decoding_operations_follow_persisted_queue_not_planning_status(has_queue, has_suspended, allowed):
    state = Session(
        ...
        backend_mode="live",
        execution="planning",
        queue=[task()] if has_queue else [],
        suspended=[[task()]] if has_suspended else [],
    )
    assert set(permitted_operations(state)) == allowed
    schema = decision_schema(state, load_scene(state.scene_id))
    exposed = {
        operation for branch in schema["anyOf"] for operation in branch["properties"]["operation"]["enum"]
    }
    assert exposed == allowed
    assert all(
        branch["additionalProperties"] is False and "cues" in branch["required"] for branch in schema["anyOf"]
    )
...
def test_decoder_rejects_all_next_roles_and_empty_plan_without_rewriting():
    ...
    assert {tuple(cue["properties"]["task_role"]["enum"]) for cue in following} == {
        ("next",),
        ("background",),
    }
...
def test_request_exposes_empty_task_state_without_leaking_expected_plan():
    ...
    body = OpenAICompatiblePlanner().request_body(
        state, spec, UserMessage(text=spec.initial_instruction, request_id="initial", expected_revision=0)
    )
    context = json.loads(body["messages"][1]["content"][0]["text"])
    ...
    assert body["temperature"] == 0.2 and body["top_p"] == 0.8 and body["max_tokens"] == 1600
...
def test_decoding_requires_structured_action_parameters_not_instruction_only(scene_id):
    ...
    for definition in ("CurrentCue", "FollowingCue"):
        by_action = {
            cue["properties"]["action"]["const"]: cue for cue in schema["$defs"][definition]["anyOf"]
        }
...
def test_background_knob_label_uses_supported_point_not_unsupported_wait():
    ...
    point = next(
        cue
        for cue in permitted
        if cue["properties"]["action"]["const"] == "point"
...
def test_superseded_uncommitted_request_cannot_become_latest_model_instruction():
    ...
    state = Session(
        ...
        scene_id="control_panel",
        backend_mode="live",
        history=[previous_user, previous_assistant, cancelled, latest],
        execution="planning",
    )
...
def test_model_schema_separates_real_question_from_absent_narrative():
    state = Session(
        session_id="question", scene_id="control_panel", backend_mode="domain_test", queue=[task()]
    )
...
def test_model_boundary_rejects_prose_without_erasing_it(operation):
    decision = Decision(
        operation=operation,
        assistant_message="Domain boundary input: retain this string.",
