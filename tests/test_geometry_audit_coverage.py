...
def test_optical_audit_keeps_completed_insertion_and_separate_mirror_steps():
    ...
    assert [(step.target_id,step.action) for _,step,_,_,_ in stages]==[
        ('L1','insert'),('M','rotate'),('M','inspect_back')]
    ...
    rotation=stages[1][3];inspection=stages[2][3]
    ...
    local=np.eye(4)
    local[:3,:3]=Rotation.from_rotvec(np.asarray(rotation.interaction.rotation_axis_local)*np.deg2rad(-15)).as_matrix()
