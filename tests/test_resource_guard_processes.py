...
def test_guard_cleans_owned_independent_process_sessions(tmp_path, termination):
    ...
    guard_command = [sys.executable, str(PROJECT / 'scripts/guard/resource_guard.py'),
                     '--rss-limit-gb', '8', '--log', str(log_path), '--execute', '--',
                     *worker, root_role, str(tmp_path)]
    ...
    try:
        ...
        with (tmp_path / 'guard_stdout.log').open('wb') as output:
            guard = subprocess.Popen(guard_command, cwd=PROJECT, stdout=output,
                                     stderr=subprocess.STDOUT, start_new_session=True)
            roles = {root_role, 'orphan'} if termination == 'quick_exit' else {root_role, 'child', 'leaf'}
            records = wait_for(lambda: identities(tmp_path),
                               lambda rows: {row['role'] for row in rows} == roles)
            ...
            if termination != 'quick_exit':
                wait_for(lambda: log_path.read_text().splitlines() if log_path.exists() else [],
        ...
        evidence = {'guard_command': guard_command, 'termination': termination,
                    'returncode': returncode, 'owned': records, 'all_owned_stopped': True,
                    'unrelated_sibling': identities(sentinel_dir)[0], 'unrelated_sibling_alive': True,
                    'guard_samples': [json.loads(line) for line in log_path.read_text().splitlines()]}
        if termination == 'quick_exit':
            evidence['quick_exit_event'] = ready
            evidence['quick_exit_scope'] = 'Immediate root exit after verified independent-session orphan spawn'
...
def record_identity(process, role, directory, observation=False):
    identity = {'role': role, 'pid': process.pid, 'create_time': process.create_time(),
                'pgid': os.getpgid(process.pid), 'ppid': process.ppid(),
                'cuda_visible_devices': process.environ().get('CUDA_VISIBLE_DEVICES'),
                'recorded_by_pid': os.getpid(), 'recorded_at': time.time(),
...
def worker(role, directory):
    ...
    if role == 'root_quick':
        child = subprocess.Popen([sys.executable, str(Path(__file__).resolve()), '--worker', 'orphan',
        ...
        ready = {'time': time.time(), 'orphan_pid': child.pid,
