...
def installation(scene_id, target, receiver):
    ...
    state = apply_decision(
        state, Decision(operation="replace", assistant_message="安装步骤", cues=[cue]), spec
    )
    return spec, state


@pytest.mark.parametrize(
    "sid,source,receiver", [("optical_bench", "L1", "POST2"), ("server_rack", "SPARE", "SLOT4")]
)
def test_confirmed_placement_updates_both_objects(sid, source, receiver):
    ...
    assert state.model_dump_json() == before
    assert packet(state, spec, load_policy()).object_poses[source] != next(
        o.pose for o in spec.objects if o.object_id == source
    )
...
def test_pause_interrupt_and_resume_keep_task_identity():
    ...
    state = apply_decision(
        state, Decision(operation="interrupt", assistant_message="检查镜架", cues=[cue]), spec
    )
...
def test_unrecognized_or_wrong_scene_is_rejected():
    ...
    with pytest.raises(KeyError):
        object_summary(spec, state, "MISSING")
    with pytest.raises(ValueError):
        object_summary(load_scene("blocks"), state, "A")
