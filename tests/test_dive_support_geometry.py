...
def test_seats_contact_boot_faces_and_clear_lower_domes(candidate, roundtrip):
    ...
    for seat, obj in zip(seats, bottles):
        ...
        assert radial.max() < obj.size_m[0] * 0.46 * np.cos(np.pi / 48) - 0.002
...
def test_bottle3_front_label_and_body_have_clear_detail_sightlines(candidate, aspect, installed):
    ...
    position, look, up = camera.detail_view(
        obj, obj.pose, local.bounds, np.asarray(spec.camera_position_m) - spec.camera_look_at_m, aspect=aspect
    )
    ...
    assert np.abs(projected).max() <= 0.860001
    poses = {}
    if installed:
    ...
    label = np.array(
        [
            (x, -0.19 * 0.493, z)
            for x in np.linspace(-0.19 * 0.75 / 2, 0.19 * 0.75 / 2, 5)
            for z in np.linspace(-0.74 * 0.15 / 2, 0.74 * 0.15 / 2, 3)
        ]
    )
    body = parts(local, "bottle_body")[0]
    samples = []
    for x in (-0.085, -0.055, 0.0, 0.055, 0.085):
        for z in (-0.26, -0.20, 0.10, 0.19, 0.24):
            origin = np.array([x, -0.2, z])
            distance = first_hit_distance(body.triangles, origin, np.array([0.0, 1.0, 0.0]))
            assert np.isfinite(distance)
            samples.append(origin + np.array([0.0, distance, 0.0]))
...
def test_original_environment_meshes_and_task_contract_are_unchanged(candidate):
    ...
    for node in original_env.graph.nodes_geometry:
        ...
        if node not in {
            "dive_fillstation_environment/0009_manifold",
            "dive_fillstation_environment/0016_tube",
        }:
            np.testing.assert_array_equal(
                env.geometry[name].vertices, original_env.geometry[old_name].vertices
            )
        ...
        for attribute in ("name", "baseColorFactor", "metallicFactor", "roughnessFactor", "doubleSided"):
            np.testing.assert_array_equal(
                getattr(env.geometry[name].visual.material, attribute),
...
def test_stem_handle_and_retaining_bolt_are_connected_without_changing_details(candidate, roundtrip):
    ...
    assert (
        hashlib.sha256(old_asset.read_bytes()).hexdigest()
        == "f0166005d4b1e490320957e6e76034f25e32a44546f4419e0ce33f2d61a5edc2"
    )
    assert (
        hashlib.sha256(old_contract.read_bytes()).hexdigest()
        == "4ad01850c4e4cf35472d425bf9143bbffad4ed004be38d5977e6dadc8f0c19a0"
    )
    ...
    assert spindle.bounds[1, 1] == pytest.approx(0.016, abs=1e-7)
    assert spindle.bounds[0, 1] == pytest.approx(-0.068, abs=1e-7)
    np.testing.assert_allclose(spindle.bounds[:, [0, 2]], [[-0.016, -0.016], [0.016, 0.016]], atol=1e-7)
    assert inside(spindle, [0.0, -0.063, 0.0]) and inside(handle, [0.0, -0.063, 0.0])
    assert inside(bolt, [0.0, -0.0748, 0.0]) and inside(handle, [0.0, -0.0748, 0.0])
