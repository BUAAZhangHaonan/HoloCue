...
@pytest.mark.parametrize(
    "scene_id,oid,inner", [("cnc_toolchange", "POCKET9", 0.020), ("engine_bay", "PLUGPORT", 0.012)]
)
@pytest.mark.parametrize("roundtrip", [False, True], ids=["generated", "glb_roundtrip"])
def test_receiver_has_closed_nonoverlapping_body_and_seating_collar(
    tmp_path, scene_id, oid, inner, roundtrip
):
    ...
    height = obj.size_m[2]
    outer = obj.size_m[0] / 2
    rim = obj.size_m[0] * 0.55
    ...
    assert seat.extents[2] == pytest.approx(0.006, abs=1e-8)
    ...
    for name, mesh in meshes.items():
        top = (np.abs(mesh.triangles[:, :, 2] - height / 2).max(axis=1) < 1e-8) & (
            mesh.face_normals[:, 2] > 0.999999
        )
        top_area[name] = float(mesh.area_faces[top].sum())
        if top.any():
            assert np.all(mesh.vertex_normals[mesh.faces[top], 2] > 0.999999)
    assert top_area["receiver_bore"] == 0
    # Exact area for the real 48-segment annulus, not an idealized smooth circle.
    assert top_area["seating_face"] == pytest.approx(
        24 * np.sin(2 * np.pi / 48) * (rim**2 - inner**2), rel=2e-6
    )
    ...
    assert body.volume + seat.volume == pytest.approx(
        area_body * (height - 0.006) + area_seat * 0.006, rel=3e-6
    )
