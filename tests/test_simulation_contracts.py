    ...
    for index, step in enumerate(spec.task_contract.ordered_steps):
        ...
        cues.append(
            CueSemantic(
                ...
                priority=5 if index == 0 else 2,
                depth_requirement=step.depth_requirement,
                instruction=step.action + " " + step.target_id,
...
def task_session(spec):
    session = Session(
        session_id="domain-" + spec.scene_id, scene_id=spec.scene_id, backend_mode="domain_test"
    )
...
@pytest.mark.parametrize("scene_id", SCENES)
@pytest.mark.parametrize("aspect", [0.72, 1.0, 16 / 9, 2.1])
def test_perspective_camera_bounds(scene_id, aspect):
    ...
    position, look, up = camera.detail_view(
        active, active.pose, mesh.bounds, direction, step.action, aspect=aspect
    )
    local_points = (
        np.asarray([active.interaction.inspect_point_local_m])
        if step.action == "inspect_back"
        else camera.corners(mesh.bounds)
    )
    ...
    interrupt = Decision(
        ...
        cues=[
            CueSemantic(
                ...
                priority=5,
                depth_requirement="persistent",
                instruction="temporary inspection " + target,
            )
        ],
    )
...
def test_motion_budget_and_final_pose(scene_id):
    ...
    while state.queue:
        ...
        assert np.allclose(matrix(trajectory(cue, 100.0)), matrix(samples[-1]), atol=1e-7)
        if cue.action in ("insert", "assemble"):
            ...
            receiver = (
                target.frames[anchor] if anchor in target.frames else Pose(position_m=target.anchors[anchor])
            )
...
def test_sqlite_recovery_and_revision(scene_id, tmp_path):
    ...
    old_job, old_state, _ = restored.begin(
        state.session_id, UserMessage(text="old", request_id="old", expected_revision=recovered.revision)
    )
    new_job, _, _ = restored.begin(
        state.session_id, UserMessage(text="new", request_id="new", expected_revision=old_state.revision)
    )
...
def test_splat_arrays_real_asset_pool(scene_id):
    ...
    for cue in display.cues:
        ...
        dark = splat_arrays(spec, cue, cue.pose, spec.camera_position_m, spec.camera_look_at_m, 1.2, 0.0)
...
def test_rotation_about_pivot():
    ...
    pose = Pose(
        ...
        wxyz=tuple(Rotation.from_euler("xyz", [0.3, 0.2, 0.4]).as_quat(scalar_first=True)),
    )
    pivot = (0.1, 0.2, 0.0)
    result = rotate_about(pose, (1.0, 0.0, 0.0), pivot, 70.0)
    assert np.allclose(
        transform_points(pose, np.asarray([pivot])), transform_points(result, np.asarray([pivot]))
    )
...
def test_clock_freezes_and_restores():
    clock = ActionClock()
    assert clock.advance("B", 0.8, True) == 0.8
    assert clock.advance("B", 5, False) == 0.8
    assert clock.advance("C", 1, True) == 1
    assert clock.advance("B", 0.2, True) == 1


def test_axial_response_relationship_and_raster():
    cal = profile()
    narrow, peak_n, z_n = envelope(1.0, np.asarray([0.0, 0.3]), cal)
    broad, peak_b, z_b = envelope(2.5, np.asarray([0.0, 0.3]), cal)
