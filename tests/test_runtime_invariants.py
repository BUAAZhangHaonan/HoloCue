...
def test_declared_workspace_fits_portrait_and_wide_views(scene_id):
    spec = load_scene(scene_id)
    points = workspace_points(spec, np.zeros((0, 3)))
    for aspect in (0.65, 1.0, 1.6, 2.2):
    ...
    decision = Decision(
        ...
        cues=[
            CueSemantic(
                ...
                instruction="移动 RED 到 BASKET",
            )
        ],
    )
    state = apply_decision(
        Session(session_id="departure", scene_id=spec.scene_id, backend_mode="domain_test"), decision, spec
    )
    ...
    decision = Decision(
        ...
        cues=[
            CueSemantic(
                ...
                task_role="current",
                depth_requirement="precise",
                instruction="逆时针转动 B 30 度",
            )
        ],
    )
    state = apply_decision(
        Session(session_id="bridge", scene_id=spec.scene_id, backend_mode="domain_test"), decision, spec
    )
    display = packet(state, spec, load_policy())
    data = frame(spec, display, ActionClock(), 0.5, spec.camera_position_m, spec.camera_look_at_m, 1.0, 0.0)
    assert len(data["cues"][0]["centers_m"]) == 6000
    assert max(data["cues"][0]["opacities"]) == 0
    assert data["rgb_encoding"] == "linear_0_1"
    assert np.allclose(data["cues"][0]["rgb"], [0.12, 0.8, 0.64])
    path = tmp_path / "actual_frame.json"
...
def test_clockwise_arrow_reverses_the_visible_arrowhead():
    ...
    semantic = CueSemantic(
        ...
        task_role="current",
        depth_requirement="precise",
        instruction="Rotate B",
    )
    state = apply_decision(
        Session(session_id="direction", scene_id=spec.scene_id, backend_mode="domain_test"),
        Decision(operation="replace", assistant_message="Rotate B", cues=[semantic]),
        spec,
    )
    cue = packet(state, spec, load_policy()).cues[0]
    positive = splat_arrays(spec, cue, cue.pose, spec.camera_position_m, spec.camera_look_at_m, 1.0)[0]
    negative = splat_arrays(
        spec,
        cue.model_copy(update={"angle_deg": -30}),
        cue.pose,
        spec.camera_position_m,
        spec.camera_look_at_m,
        1.0,
    )[0]
    plane = basis_from_normal(cue.interaction.rotation_axis_local)
    origin = np.asarray(cue.interaction.cue_offset_local_m)
    first = (positive - origin) @ plane
    second = (negative - origin) @ plane
