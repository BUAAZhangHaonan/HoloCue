...
HASHES = {
    "L1.glb": "dd4236403f6b3cf62516d168f858b544fe7d32fd93d17f08eac6dd09b977230b",
...
def nodes(scene, pose=np.eye(4)):
    ...
    for node in scene.graph.nodes_geometry:
        ...
        mesh.apply_transform(pose @ transform)
...
def hits(mesh, origin, direction):
    ...
    selected = np.flatnonzero(
        (abs(det) > 1e-12) & (u >= -1e-9) & (v >= -1e-9) & (u + v <= 1 + 1e-9) & (t > 1e-9)
    )
...
def lenses(tmp_path_factory):
    ...
    directory = tmp_path_factory.mktemp("optical-lens-seats")
    result = {}
    for oid in ("L1", "L2"):
        obj = next(o for o in spec.objects if o.object_id == oid)
        path = directory / (oid + ".glb")
        make_object(obj).save(path)
        result[oid] = (
            nodes(load_asset(str(FIXTURE / (oid + ".glb")), spec.asset_axes)),
            nodes(load_asset(str(path), spec.asset_axes)),
            path,
        )
...
def test_only_retainer_and_post_change_with_all_materials_preserved(lenses, oid):
    ...
    for name, before in old.items():
        ...
        if not equal:
            ...
            np.testing.assert_array_equal(before.vertex_normals, after.vertex_normals)
        for key in (
            "name",
            "baseColorFactor",
            "metallicFactor",
            ...
            np.testing.assert_array_equal(
                getattr(before.visual.material, key), getattr(after.visual.material, key)
            )
...
def test_real_blind_seat_walls_and_post_end_contact(lenses, oid):
    _, data, directory = lenses
    _, new, _ = data[oid]
    ring = closed(new[oid + "/0000_optic_retainer"])
    post = closed(new[oid + "/0002_mounting_post"])
    assert post.vertices[:, 2].min() == pytest.approx(-0.065, abs=1e-7)
    assert post.vertices[:, 2].max() == pytest.approx(-0.020, abs=1e-7)
    np.testing.assert_allclose(post.bounds[:, :2], [[-0.007, -0.007], [0.007, 0.007]], atol=1e-7)
    cap = post.triangles[np.max(abs(post.triangles[:, :, 2] + 0.020), axis=1) < 1e-7]
    assert len(cap) == 40
    closest, distance, faces = trimesh.proximity.closest_point_naive(ring, cap.mean(axis=1))
    assert distance.max() < 1e-7
    axis = hits(ring, np.array([0.0, 0.0, -0.070]), np.array([0.0, 0.0, 1.0]))
    z = np.unique(np.round([value - 0.070 for value, _ in axis], 9))
    assert z[0] == pytest.approx(-0.020, abs=1e-7)
    assert z[1] - z[0] > 0.0067
    rays = []
    for height in (-0.024, -0.022, -0.0205):
        for theta in np.linspace(0, 2 * np.pi, 80, endpoint=False):
            direction = np.array([np.cos(theta), np.sin(theta), 0.0])
            values = np.unique(
                np.round([value for value, _ in hits(ring, np.array([0.0, 0.0, height]), direction)], 9)
            )
            assert len(values) >= 2 and 0.007075 < values[0] < 0.007101
            assert values[1] - values[0] > 0.0063
            rays.append({"z": height, "direction": direction.tolist(), "intersections": values.tolist()})
    report = {
        "cap_triangles": cap.tolist(),
        "nearest_ring_points": closest.tolist(),
        "ring_face_indices": faces.tolist(),
        "nearest_ring_triangles": ring.triangles[faces].tolist(),
        "contact_distances": distance.tolist(),
        "axis_intersections_z": z.tolist(),
        "radial_rays": rays,
        "ring_volume_m3": float(ring.volume),
    }
...
def test_post_clears_all_lens_triangles_and_preserves_the_lower_insertion_end(lenses, oid):
    _, data, _ = lenses
    old, new, _ = data[oid]
    post = new[oid + "/0002_mounting_post"]
    lens = new[oid + "/0001_convex_lens"]
    before = old[oid + "/0002_mounting_post"]
    ...
    assert lens.vertices[:, 2].min() - post.vertices[:, 2].max() > 0.0057
    assert distances(lens, post.vertices).min() > 0.0057
    old_bottom = before.vertices[abs(before.vertices[:, 2] + 0.065) < 1e-7]
    bottom = post.vertices[abs(post.vertices[:, 2] + 0.065) < 1e-7]
...
def test_original_retainer_surface_outside_the_local_blind_seat_is_preserved(lenses, oid):
    ...
    for source, target in (
        (old[oid + "/0000_optic_retainer"], new[oid + "/0000_optic_retainer"]),
        (new[oid + "/0000_optic_retainer"], old[oid + "/0000_optic_retainer"]),
    ):
        samples = np.concatenate([source.vertices, source.triangles_center])
        untouched = (np.linalg.norm(samples[:, :2], axis=1) > 0.0072) | (samples[:, 2] > -0.0199)
...
def test_optical_aperture_and_archived_camera_rays_hit_the_lens_not_the_post(lenses, oid):
    spec, data, directory = lenses
    old, new, path = data[oid]
    for x in (-0.004, 0.0, 0.004):
        for z in (-0.012, -0.009, -0.006, -0.003, -0.0005):
            origin = np.array([x, -0.1, z])
            direction = np.array([0.0, 1.0, 0.0])
            nearest = []
            for name, mesh in new.items():
                value = hits(mesh, origin, direction)
                if value:
                    nearest.append((value[0][0], name))
            assert min(nearest)[1] == oid + "/0001_convex_lens"
    frame = json.loads((FIXTURE / (oid + "_view.json")).read_text())
    world = {}
    for obj in spec.objects:
        asset = path if obj.object_id == oid else root() / obj.asset
        world.update(
            nodes(
                load_asset(str(asset), spec.asset_axes),
                matrix(Pose.model_validate(frame["object_poses"][obj.object_id])),
            )
        )
    for prop in spec.environment:
        world.update(
            nodes(
                load_asset(str(root() / prop.asset), spec.asset_axes),
                matrix(prop.pose) @ np.diag([*prop.scale_m, 1.0]),
            )
        )
    original_rays = json.loads((FIXTURE / "visible_rays.json").read_text())[oid]
    result = []
    assert len(original_rays) == 8
    for ray in original_rays:
        origin = np.asarray(ray["camera_world"])
        direction = np.asarray(ray["direction_world"])
        nearest = []
        for name, mesh in world.items():
            value = hits(mesh, origin, direction)
            if value:
                nearest.append((value[0][0], name, value[0][1]))
        ...
        result.append(
            {
                ...
                "old_first_node": ray["world_first_hit_node"],
                "new_first_node": name,
                "distance_m": distance,
                "new_face_index": face,
                "new_triangle": world[name].triangles[face].tolist(),
            }
        )
