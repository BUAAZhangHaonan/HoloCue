...
# Immutable inputs from c4463b3a9842b3945ab7d1582ab8e49fcadca245.
# Preserve the same byte assertions without requiring a generated checksum file.
REFERENCE = {
    "BAY": {
        "scene_id": "drone_bench",
        "sha256": "95e64ef776d73c71c5b976295dc5ac198abdb8b1b9c8ff58ebbfc87923527209",
    },
    "BLUE": {
        "scene_id": "shelf_picking",
        "sha256": "450752216abac2a7d39d1ea006b139b358541c483821fbeebfb0b5bbefa073fa",
    },
    "BASKET": {
        "scene_id": "shelf_picking",
        "sha256": "d6b363965c8457c69e37b6ca3d3411a246b4454986f29c2c87dda21f439b0b61",
    },
    "workstation": {
        "scene_id": "engine_bay",
        "sha256": "97a90be4711af3718ae68e5fe88482248c5f94719d075329c01676d53488b5e1",
    },
}
...
def first_hit(trees, start, end):
    ...
    for tree in trees:
        ...
        distances.extend(
            np.linalg.norm(np.asarray(points.GetPoint(i)) - start) for i in range(points.GetNumberOfPoints())
        )
...
def assert_exterior_rays(old, new, tolerance_m=3e-6):
    ...
    for axis in range(3):
        ...
        for u, v in itertools.product(np.linspace(0.10, 0.90, 7), repeat=2):
            point = (lo + hi) / 2
            point[others] = lo[others] + np.asarray([u, v]) * (hi - lo)[others]
            for sign in (-1, 1):
                start = point.copy()
                end = point.copy()
                start[axis] = lo[axis] - 0.10 if sign == 1 else hi[axis] + 0.10
                end[axis] = hi[axis] + 0.10 if sign == 1 else lo[axis] - 0.10
    ...
    return {
        "ray_hits": len(observed),
        "max_distance_change_m": float(max(observed)),
...
def assert_container_partition(spec, obj, actual):
    ...
    assert_exterior_rays(
        {key: old[key] for key in expected_nodes}, {key: actual[key] for key in expected_nodes}
    )


@pytest.mark.parametrize(
    "sid,oid", [("drone_bench", "BAY"), ("shelf_picking", "BLUE"), ("shelf_picking", "BASKET")]
)
...
def test_cad_partition_conserves_union_volume_and_has_disjoint_interiors(oid, tmp_path):
    if oid == "workstation":
        ...
        shapes = [
            cq.Workplane("XY").box(*size).edges().fillet(min(min(size) * 0.16, 0.006)).translate(position)
            for size, position in panels
        ]
    regions = list(partition_cad_parts(shapes))
    whole = shapes[0]
    for shape in shapes[1:]:
        whole = whole.union(shape)
    ...
    (tmp_path / (oid + "_cad_conservation.json")).write_text(
        json.dumps(
            {
                "missing_solids": 0,
                "added_solids": 0,
                "partition_volume_m3": total,
                "union_volume_m3": whole.val().Volume(tol=1e-12),
                "volume_integration_difference_m3": abs(total - whole.val().Volume(tol=1e-12)),
            },
            indent=2,
        )
    )
...
def test_exported_engine_exterior_rays_and_unaffected_components(tmp_path):
    ...
    result = assert_exterior_rays(
        {key: old[key] for key in parts}, {key: new[key] for key in parts}, CAD_TESSELLATION_M
    )
