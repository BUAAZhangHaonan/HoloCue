...
def say(text):
 j,s,elapsed=send(client,sid,text,timeout=100,poll=.25)
 report['events'].append({'input':text,'job':j,'elapsed_s':elapsed,'state':s});save()
 assert j['status']=='done',j
 return state(client,sid)
try:
 assert req(client,'GET','/health')['backend_mode']=='live','Live endpoint required'
 sid=new_session(client,'control_panel');report['session_id']=sid
 ...
 before=s['queue'];s=control(client,sid,'pause')
