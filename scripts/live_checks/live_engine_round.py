...
def req(method, url, **kw):
    return _req(client, method, url, **kw)


def state(sid):
    return _state(client, sid)


def display(sid):
    return _display(client, sid)


def control(sid, operation):
    return _control(client, sid, operation)
            ...
            "cues": [
                {
                    ...
                    "action": c["action"],
                    "task_role": c["task_role"],
                    "cue_type": c["cue_type"],
                    "angle_deg": c["angle_deg"],
                    "reference_id": c["reference_id"],
                    "depth_requirement": c["depth_requirement"],
                    "priority": c["priority"],
                    "n_gaussians": c["n_gaussians"],
                    "sigma_value": c["sigma_value"],
                    "sigma_profile": c["sigma_profile"],
                }
                for c in d["cues"]
            ],
        },
    ...
    steps.append(
        {
            ...
            "raw_http_body": (
                (j.get("data") or {}).get("raw_http_body")
                if (j.get("data") or {}).get("error") or j["status"] == "error"
                else None
            ),
...
ok_all &= check(
    steps, "operation_replace", dec.get("operation") == "replace", f"operation={dec.get('operation')}"
)
ok_all &= check(steps, "at_least_2_cues", len(cs) >= 2, f"n_cues={len(cs)}")
first = cs[0] if cs else {}
ok_all &= check(
    steps,
    "first_cue_current_CLAMP_rotate_90",
    first.get("target_id") == "CLAMP"
    and first.get("action") == "rotate"
    and first.get("task_role") == "current"
    and first.get("angle_deg") == 90,
    f"first={first}",
)
plug = next((c for c in cs if c.get("target_id") == "PLUG"), None)
ok_all &= check(
    steps,
    "PLUG_insert_ref_PLUGPORT",
    plug and plug.get("action") == "insert" and plug.get("reference_id") == "PLUGPORT",
    f"plug={plug}",
)
conn = next((c for c in cs if c.get("target_id") == "CONN"), None)
ok_all &= check(steps, "CONN_inspect_back", conn and conn.get("action") == "inspect_back", f"conn={conn}")
dc = d["cues"]
...
ok_all = check(steps, "job_done", j["status"] == "done", f"job={j['status']}")
ok_all &= check(
    steps, "operation_interrupt", dec.get("operation") == "interrupt", f"operation={dec.get('operation')}"
)
ok_all &= check(
    steps,
    "suspended_depth_1_with_CLAMP",
    len(s["suspended"]) == 1 and any(t["semantic"]["target_id"] == "CLAMP" for t in s["suspended"][0]),
    f"suspended={[[t['semantic']['target_id'] for t in q] for q in s['suspended']]}",
)
q0 = cues_of(s)[0] if s["queue"] else {}
ok_all &= check(
    steps,
    "new_cue_CONN_current",
...
steps[-1]["note"] = (
    "model chose resume while the temporary CONN task was still in the queue; the domain guard rejected it"
    if blocked
    else ""
)
...
if literal_ok:
    ...
    literal_ok &= check(
        steps, "suspended_popped", len(s["suspended"]) == 0, f"suspended={len(s['suspended'])}"
    )
    literal_ok &= check(
        steps, "CLAMP_back_angle90", clamp is not None and clamp.get("angle_deg") == 90, f"clamp={clamp}"
    )
    record("E3_resume", e1_sid, E3, steps, ("passed" if literal_ok else "FAILED", "literal single message"))
else:
    check(
        steps,
        "resume_blocked_by_design",
    ...
    ok2 = check(
        steps, "operation_resume", dec2.get("operation") == "resume", f"operation={dec2.get('operation')}"
    )
    ok2 &= check(steps, "suspended_popped", len(s2["suspended"]) == 0, f"suspended={len(s2['suspended'])}")
    ok2 &= check(
        steps, "CLAMP_back_angle90", clamp is not None and clamp.get("angle_deg") == 90, f"clamp={clamp}"
    )
    record(
...
ok_all &= check(steps, "revision_advanced", s["revision"] > rev0, f"rev {rev0}->{s['revision']}")
...
ok_all &= check(
    steps,
    "old_90_cue_gone_from_display",
    not any(c["target_id"] == "CLAMP" and c["angle_deg"] == 90 for c in d["cues"])
    and not any(
        c["target_id"] == "CLAMP" and c["angle_deg"] == 90 for t in s["queue"] for c in [t["semantic"]]
    ),
    f"display_clamp={[(c['target_id'], c['angle_deg']) for c in d['cues']]}",
)
ok_all &= check(
    steps, "suspended_cleared_by_replace", len(s["suspended"]) == 0, f"suspended={len(s['suspended'])}"
)
ok_all &= check(
    steps,
    "new_plan_is_current_version",
...
if j["status"] == "error":
    ...
    ok_all = check(
        steps,
        "visible_error_PLUG_rotate",
        s.get("last_error") is not None and "PLUG" in s["last_error"] and "rotate" in s["last_error"],
        f"last_error={s.get('last_error')}",
    )
    ok_all &= check(
        steps,
        "job_data_error_kept",
        (j.get("data") or {}).get("error") is not None
    ...
    ok_all = check(steps, "model_refused_violating_cue", not violating, f"cues={cs}")
else:
    ok_all = check(steps, "terminal_status", False, f"job={j['status']}")
ok_all &= check(
    steps,
    "no_violating_cue_in_display",
    not disp_violating,
    f"display_plug={[c for c in d['cues'] if c['target_id'] == 'PLUG']}",
)
ok_all &= check(
    steps,
    "session_readable_revision_intact",
