...
def cues_of(s):
    ...
    return [(t['semantic']['target_id'], t['semantic']['action'], t['semantic']['task_role'])
            for t in tasks]


def snap(job, s, d, elapsed):
    return {
        'job_status': job['status'], 'latency_s': round(elapsed, 2),
...
SCENES = {
    'cnc_toolchange': {
        'init': ('先把模式旋钮 MODESWITCH 顺时针拧 90 度切到手动档,把镗刀 T09 插进刀库当前的 '
    ...
    'dive_fillstation': {
        'init': ('把充装软管接头 QRC 接到 3 号气瓶瓶阀 VALVE 的出口上,接好后核对 BOTTLE3 瓶肩的'
    ...
    'infusion_ward': {
        'init': ('把新泵盒 PUMPCASSETTE 插进输液泵的 2 号泵槽 SLOT2,插好后核对 BAG1 背面的配置'
...
for scene, spec in SCENES.items():
    ...
    try:
        ...
        report['cases'].append({'scene': scene, 'name': 'E1_initial_plan', 'input': spec['init'],
                                'verdict': 'HARNESS_ERROR', 'note': repr(e), 'traceback': __import__('traceback').format_exc()})
        save()
        continue
    ...
    try:
        ...
        v, n = ok(s['execution'] == want_exec and (not q or q[0] == want_tgt)
                  and len(s['suspended']) >= 1,
                  f'execution={s["execution"]} queue={q} suspended={len(s["suspended"])}')
        record('E2_interrupt', spec['interrupt'], job, s, d, el, v, n)
    except Exception as e:  # noqa: BLE001
        report['cases'].append({'scene': scene, 'name': 'E2_interrupt', 'input': spec['interrupt'],
                                'verdict': 'HARNESS_ERROR', 'note': repr(e), 'traceback': __import__('traceback').format_exc()})
        save()
        continue
    ...
    try:
        ...
        v, n = ok(job['status'] == 'done' and not s['queue'] and len(s['suspended']) == 1,
        ...
        q = [t['semantic']['target_id'] for t in s['queue']]
        v, n = ok(s['execution'] == want_exec and q and want_tgt in q,
                  f'execution={s["execution"]} queue={q} suspended={len(s["suspended"])}')
        record('E3_resume_restore', '继续。', job, s, d, el, v, n)
    except Exception as e:  # noqa: BLE001
        report['cases'].append({'scene': scene, 'name': 'E3_resume', 'input': spec['resume'],
                                'verdict': 'HARNESS_ERROR', 'note': repr(e), 'traceback': __import__('traceback').format_exc()})
    ...
    try:
        ...
        v, n = ok(job['status'] == 'done' and not invented and not s.get('last_error'),
        ...
        report['cases'].append({'scene': scene, 'name': 'E4_anchor_unique', 'input': spec['anchor'],
                                'verdict': 'HARNESS_ERROR', 'note': repr(e), 'traceback': __import__('traceback').format_exc()})
    ...
    try:
        ...
        v, n = ok(job['status'] != 'planning' and not new_rotate,
        ...
        report['cases'].append({'scene': scene, 'name': 'E5_capability_violation', 'input': spec['violation'],
                                'verdict': 'HARNESS_ERROR', 'note': repr(e), 'traceback': __import__('traceback').format_exc()})

verdicts = [c['verdict'] for c in report['cases']]
report['summary'] = {'total': len(verdicts),
