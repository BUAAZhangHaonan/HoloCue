...
p.add_argument("--out", default="runs/live_extended.json")
a = p.parse_args()
...
def req(method, path, **kwargs):
    return _req(client, method, path, **kwargs)
...
def say(sid, text):
    return _send(client, sid, text, timeout=120)
...
case(
    "nl_task_start",
    g1,
    "把 B 逆时针转 30 度，接着检查 C 的背面。",
    lambda j, s: ok(
        j["status"] == "done" and s["queue"] and s["queue"][0]["semantic"]["target_id"] == "B",
...
case(
    ...
    g1,
    "暂停一下，先别继续",
    lambda j, s: ok(s["execution"] == "paused", f"execution={s['execution']}"),
)
case(
    "nl_resume",
    g1,
    "继续刚才没做完的那步",
    lambda j, s: ok(
        s["execution"] == "running" and s["queue"] and s["queue"][0]["semantic"]["target_id"] == "B",
...
def c2(j, s):
    q = s["queue"]
    b = next((t for t in q if t["semantic"]["target_id"] == "B"), None)
    return ok(
        b is not None and b["semantic"].get("angle_deg") == 15 and q[0]["semantic"]["target_id"] == "B",
...
case(
    ...
    "把 B 逆时针转 30 度，接着检查 C 的背面。",
    lambda j, s: ok(
        j["status"] == "done" and s["queue"] and s["queue"][0]["semantic"]["target_id"] == "B", "start ok"
    ),
)
...
def c3(j, s):
    return ok(
        len(s["suspended"]) >= 1
        and any(t["semantic"]["target_id"] == "B" for q2 in s["suspended"] for t in q2)
...
case(
    ...
    "把刚才临时检查的任务结束掉",
    lambda j, s: ok(
        any(t["semantic"]["target_id"] == "C" for t in s["completed"]) and not s["queue"],
        f"completed={[t['semantic']['target_id'] for t in s['completed']]}",
    ),
)
case(
    "resume_after_complete",
    g3,
    "继续刚才 B 的任务。",
...
def c4(j, s):
    ...
    return ok(
        d.get("operation") == "clarify" or s["last_error"] is not None,
...
def c5(j, s):
    ...
    return ok(
        clarified or rejected, f"operation={d.get('operation')} error={(s.get('last_error') or '')[:60]}"
    )
...
case(
    ...
    g7,
    "把 B 逆时针转 30 度，接着检查 C 的背面。",
    lambda j, s: ok(j["status"] == "done", "start ok"),
)


def c7(j, s):
    return ok(
        (not s["queue"] or s["queue"][0]["semantic"]["target_id"] == "C") and len(s["suspended"]) >= 1,
...
def c8(j, s):
    return ok(
        any(t["semantic"]["target_id"] == "A" for t in s["queue"]),
        f"queue={[t['semantic']['target_id'] for t in s['queue']]} suspended={len(s['suspended'])}",
    )
...
j1 = req(
    "POST",
    f"/api/v1/sessions/{g9}/messages",
    json={
        "text": "把 B 逆时针转 30 度，接着检查 C 的背面。",
...
while time.perf_counter() - t0 < 120:
    a1 = req("GET", "/api/v1/jobs/" + j1["id"])
    a2 = req("GET", "/api/v1/jobs/" + j2["id"])
    if a1["status"] != "planning" and a2["status"] != "planning":
        break
    time.sleep(0.3)
superseded = a1["status"] == "superseded"
committed = a2["status"] == "done"
report["cases"].append(
    {
        "name": "rapid_double_send",
        "job1": a1["status"],
        "job2": a2["status"],
        "verdict": "passed" if (superseded or a1["status"] == "error") and committed else "FAILED_EPOCH_RACE",
        "note": "first request must not commit after second epoch",
    }
)
save()
...
report["cases"].append(
    {
        "name": "model_down",
        "howto": "stop vLLM, send a message, expect job=error and session.last_error visible in UI/log",
        "verdict": "MANUAL_STEP",
    }
)
save()
...
report["summary"] = {
    ...
    "model_failures": sum(
        1 for c in report["cases"] if c["verdict"] in ("FAILED_MODEL_BEHAVIOR", "FAILED_EPOCH_RACE")
    ),
