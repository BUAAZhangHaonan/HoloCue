...
def gpu_inventory():
    ...
    return [
        ...
        for x in csv.reader(io.StringIO(r.stdout))
    ]
...
def gpu_jobs():
    ...
    for row in csv.reader(io.StringIO(r.stdout)):
        if row:
            jobs.append({"uuid": row[0].strip(), "pid": int(row[1])})
    return jobs
...
def main():
    ...
    p.add_argument("--host-reserve-gb", type=float, default=32)
    p.add_argument(
        "--host-max-used-fraction",
        type=float,
        default=os.environ.get("HOLOCUE_HOST_MAX_USED_FRACTION"),
        help="Explicit host policy override, inherited by nested guards through HOLOCUE_HOST_MAX_USED_FRACTION; at most 0.90",
    )
    p.add_argument("--rss-limit-gb", type=float, default=32)
    p.add_argument("--gpu-fraction", type=float, default=0.70)
    p.add_argument("--min-gpu-free-gb", type=float, default=12)
    p.add_argument("--log", default="runs/resource_guard.jsonl")
    p.add_argument("--execute", action="store_true")
    p.add_argument("command", nargs=argparse.REMAINDER)
    a = p.parse_args()
    selected = parse_selection(a.gpus)
    if not 0 < a.gpu_fraction <= 0.80:
        raise SystemExit("GPU allocation fraction must be in (0,0.80]")
    vm = psutil.virtual_memory()
    reserve = host_memory_reserve(vm.total, a.host_reserve_gb, a.host_max_used_fraction)
    if vm.available < reserve:
        raise SystemExit("Host available memory is below the reserve")
    if a.rss_limit_gb <= 0 or a.rss_limit_gb * GIB > vm.total - reserve:
        raise SystemExit("Invalid process-tree RSS limit")
    devices = [d for d in gpu_inventory() if d["index"] in selected] if selected else []
    if len(devices) != len(selected):
        raise SystemExit("Selected physical GPU does not exist")
    uuids = [d["uuid"] for d in sorted(devices, key=lambda d: selected.index(d["index"]))]
    if selected:
        occupied = [x for x in gpu_jobs() if x["uuid"] in uuids]
        if occupied:
            raise SystemExit(
                "Selected GPU already has compute jobs; do not kill them. " + json.dumps(occupied)
            )
        for d in devices:
            free = (d["total_mib"] - d["used_mib"]) * 1024**2
            if free < a.min_gpu_free_gb * GIB:
                raise SystemExit("Insufficient free GPU memory")
            if d["used_mib"] + a.gpu_fraction * d["total_mib"] > 0.90 * d["total_mib"]:
                raise SystemExit("Requested fraction would cross the GPU reserve")
    locks = []
    for d in devices:
        lp = Path("runs/locks") / (d["uuid"] + ".lock")
    ...
    info = {
        "authorized_physical_gpus": selected,
        "cuda_visible_devices": uuids,
        "host_reserve_gib": reserve / GIB,
        "rss_limit_gib": a.rss_limit_gb,
        "command": command,
        "host_total_gib": vm.total / GIB,
        "host_max_used_fraction": a.host_max_used_fraction,
        "host_reserve_policy": "explicit-used-fraction"
        if a.host_max_used_fraction is not None
        else "default-minimum-and-20-percent",
    }
    ...
    if not selected:
        ...
        if not mesa.is_file():
            raise SystemExit("CPU rendering requires the existing Mesa EGL vendor")
    ...
    try:
        while proc.poll() is None:
            ...
            rec = {
                ...
                "pid": proc.pid,
                "rss_gib": rss / GIB,
                "host_available_gib": vm.available / GIB,
                "gpus": gpus,
            }
            with log.open("a") as f:
                f.write(json.dumps(rec) + "\n")
            if vm.available < reserve:
                reason = "host memory reserve reached"
            if rss > a.rss_limit_gb * GIB:
                reason = "owned process-tree RSS limit reached"
            if any(d["used_mib"] > 0.90 * d["total_mib"] for d in gpus):
                reason = "GPU memory reserve reached"
            if reason:
                break
        ...
        if proc.poll() is None:
            proc.wait(timeout=5)
    if reason:
        raise SystemExit("Stopped only owned process group: " + reason)
