...
def main():
    ...
    def run(sid):
        with (output/f'{sid}.log').open('w') as log:
            subprocess.run([sys.executable,'scripts/scenes/validate_renderings.py','--scenes',sid,
                            '--out',str((output/sid).relative_to(root()))],cwd=root(),stdout=log,stderr=subprocess.STDOUT,check=True)
