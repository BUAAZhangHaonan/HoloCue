...
def main():
    ...
    provenance={'requested_scene_ids':selected,'built_scene_ids':[], 'built_assets':[],
                'scene_inputs':{},'disk_inventory':[], 'complete':False}
    ...
    for sid in selected:
        ...
        for obj in spec.objects:
            ...
            path=root()/obj.asset
            assembly.save(path)
            records.append({'scene_id':sid,'object_id':obj.object_id,'path':obj.asset,
                'parts':len(assembly.scene.geometry),'triangles':sum(len(m.faces) for m in assembly.scene.geometry.values()),
        ...
        env.save(path)
        records.append({'scene_id':sid,'object_id':'environment','path':spec.environment[0].asset,
            'parts':len(env.scene.geometry),'triangles':sum(len(m.faces) for m in env.scene.geometry.values()),
            'sha256':hashlib.sha256(path.read_bytes()).hexdigest()})
        provenance['built_assets']=records
    ...
    for entry in list_scenes():
        ...
        for obj in [*spec.objects,*spec.environment]:
            ...
            records.append({'scene_id':spec.scene_id,'object_id':identifier,'path':obj.asset,
                'parts':len(scene.geometry),'triangles':sum(len(m.faces) for m in scene.geometry.values()),
                'sha256':hashlib.sha256(path.read_bytes()).hexdigest()})
