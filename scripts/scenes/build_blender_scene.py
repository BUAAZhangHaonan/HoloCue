...
def main():
    ...
    position = Vector(payload["camera"]["position_m"])
    look = Vector(payload["camera"]["look_at_m"])
    camera.location = position
    camera.rotation_mode = "QUATERNION"
    camera.rotation_quaternion = (look - position).to_track_quat("-Z", "Y")
    ...
    scene.world.node_tree.nodes["Background"].inputs["Color"].default_value = (0.70, 0.76, 0.82, 1)
    scene.world.node_tree.nodes["Background"].inputs["Strength"].default_value = 0.45
    radius = max((position - look).length, 0.5)
    for index, (offset, energy, color) in enumerate(
        [
            ((1, -1.2, 2), 15, (1.0, 0.94, 0.84)),
            ((-1, -0.2, 1.4), 10, (0.82, 0.91, 1.0)),
            ((0, 1, 1.8), 12, (1.0, 1.0, 1.0)),
        ]
    ):
        data = bpy.data.lights.new("Softbox_" + str(index), "AREA")
        light = bpy.data.objects.new("Softbox_" + str(index), data)
