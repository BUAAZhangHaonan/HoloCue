...
def caption(image,text):
    out=Image.new('RGB',(image.width,image.height+55),(239,243,246));out.paste(image,(0,55))
    draw=ImageDraw.Draw(out);font=ImageFont.truetype('DejaVuSans.ttf',24)
...
def main():
    ...
    rows=[];covers=[]
    for sid in selected:
        spec=load_scene(sid);full=world_scene(root(),spec)
        points=[]
        ...
        image.save(output/f'{sid}_overview.png');covers.append((sid,image))
        front,frontlook=camera.fit(framing,(0,-1,.40),aspect=1280/900)
        ...
        dp,dl,du=camera.detail_view(active,active.pose,active_asset.bounds,direction,step.action,
                                  aspect=1280/900)
        caption(render(full,dp,dl,up=du),sid+'   |   '+active.object_id+' task detail').save(output/f'{sid}_detail.png')
        ...
        caption(render(asset,ip,il,up=up),sid+'   |   '+focus.object_id+' inspection').save(output/f'{sid}_inspection.png')
        entry={'scene_id':sid,'objects':len(spec.objects),'frustum_max_abs_ndc':float(np.abs(ndc).max()),
                     'depth_range_m':[float(depth.min()),float(depth.max())],
                     'inspection_target':focus.object_id,'renderer':'VTK OSMesa llvmpipe CPU',
                     'detail_target':active.object_id,'detail':f'{sid}_detail.png',
                     'overview':f'{sid}_overview.png','inspection':f'{sid}_inspection.png'}
        rows.append(entry)
    ...
    for index,(_,image) in enumerate(covers):
        image.thumbnail((480,375));contact.paste(image,((index%3)*480,(index//3)*380))
