...
def annotated(image,text):
    ...
    out.paste(image,(0,40));draw=ImageDraw.Draw(out)
...
def response_evidence(output):
    ...
    decision=Decision(operation='replace',assistant_message='Evaluate display response',cues=[CueSemantic(
        target_id='B',action='rotate',angle_deg=30,cue_type='ring_arrow',task_role='current',
        priority=5,depth_requirement='precise',instruction='Rotate B by 30 degrees')])
    state=apply_decision(Session(session_id='optical-envelope',scene_id=spec.scene_id,
                                backend_mode='analytical_validation'),decision,spec)
    cue=packet(state,spec,load_policy()).cues[0]
    look=np.asarray(cue.pose.position_m)+np.array([0,-.022,.042])
    position=look+np.array([.05,-.27,.27])
    ...
    def image(sigma,defocus,brightness):
        ...
        centers,cov,rgb,opacity=splat_arrays(spec,configured,configured.pose,
            position,look,focal_distance+defocus,brightness)
        rgba=Image.fromarray(camera_aligned_rgba(centers,cov,opacity,rgb,configured.pose,
            position,look,size=(480,320)))
        background=Image.new('RGBA',rgba.size,(24,32,39,255))
        background.alpha_composite(rgba)
        return background.convert('RGB'),float(opacity.sum())
    sheet=Image.new('RGB',(1440,1080),(234,240,244));measurements=[]
    for row,sigma in enumerate((1.,2.5,4.)):
        for column,defocus in enumerate((0.,.12,.35)):
            raster,alpha=image(sigma,defocus,1.)
            ...
            measurements.append({'sigma':sigma,'defocus_m':defocus,'waist_width_m':float(width),
                'peak':float(peak),'rayleigh_m':rayleigh,'sum_splat_alpha':alpha})
    sheet.save(output/'response_focus_sigma.png')
    brightness_sheet=Image.new('RGB',(1440,360),(234,240,244))
    brightness_images=[]
    for column,value in enumerate((0.,.25,1.)):
        raster,_=image(1.,0.,value);brightness_images.append(np.asarray(raster))
        brightness_sheet.paste(annotated(raster,f'brightness {value:g}'),(column*480,0))
    if not all(np.abs(brightness_images[i+1].astype(float)-brightness_images[i]).mean()>0
               for i in range(2)):
        raise AssertionError('brightness did not change the actual raster')
    brightness_sheet.save(output/'response_brightness.png')
    frames=[]
    for value in np.linspace(-.05,.40,32):
        raster,_=image(1.,float(value),1.)
        frames.append(annotated(raster,f'Analytical focus scan   offset {value:+.3f} m'))
    frames[0].save(output/'response_focus_scan.gif',save_all=True,append_images=frames[1:],
                   duration=100,loop=0)
    (output/'response_measurements.json').write_text(json.dumps({'profile':profile().model_dump(),
        'input_boundary':'declared display parameters','model_backend_exercised':False,
        'renderer':'reference raster of actual Gaussian arrays','measurements':measurements},indent=2))


def main():
    output=root()/'runs/simulation/rendered';output.mkdir(parents=True,exist_ok=True)
    ...
    for suffix in ('overview','front','detail','inspection'):
        ...
        for index,sid in enumerate(ids):
            ...
            image.thumbnail((640,475));sheet.paste(image,((index%3)*640,(index//3)*480))
        name='twelve_scenes.png' if suffix=='overview' else f'twelve_{suffix}.png'
        sheet.save(output/name)
    cards=[]
    for sid in ids:
        spec=load_scene(sid)
        images=''.join(f'<a href="{sid}/{sid}_{view}.png"><img src="{sid}/{sid}_{view}.png" alt="{sid} {view}"></a>'
                       for view in ('overview','front','detail','inspection'))
        steps=''.join(f'<li>{html.escape(s.target_id)} {html.escape(s.action)} '
                      f'{html.escape(s.reference_id or "")}</li>' for s in spec.task_contract.ordered_steps)
        cards.append(f'<section><h2>{html.escape(spec.title)}</h2><p>{html.escape(spec.task_contract.setting)}</p>'
