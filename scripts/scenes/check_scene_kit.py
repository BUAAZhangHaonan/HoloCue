...
def check(scene_id,require_blender=False):
    ...
    return {'scene_id':scene_id,'fingerprint':scene_fingerprint(scene),'objects':len(scene.objects),
            'native_blender_required':require_blender}


def main():
    parser=argparse.ArgumentParser();parser.add_argument('scenes',nargs='*')
    parser.add_argument('--require-blender',action='store_true');args=parser.parse_args()
