...
const GaussianSplatMaterial = /* @__PURE__ */ shaderMaterial(
  ...
  void main () {
    ...
    // HoloCue: retain positive-opacity points at every projected size.
    // Dense analytical cues can be visible in aggregate even when each point
    // is below the upstream size heuristic. Keep the original input arrays.
    if (vRgba.a <= 0.0)
      return;
...
`,
  ...
  void main () {
    ...
    // Preserve positive analytical opacity within the existing support domain.
    if (B <= 0.0) discard;
