...
p.add_argument(
    "--source",
    default="assets/downloads/polyhaven/models",
    help="batch root relative to repo; ids live in <source>/<id>/ or <source>/<cat>/<id>/",
)
p.add_argument(
    "--ids", nargs="*", default=None, help="restrict to these ids (default: every id found under --source)"
)
...
for mid in ids:
    ...
    bpy.ops.export_scene.gltf(
        filepath=str(out), export_format="GLB", export_apply=True, export_yup=False, use_selection=False
    )
    # dims in the STORED Z-up frame (the frame place_prop and the JSON pose use)
    dims = [round(hi[i] - lo[i], 4) for i in range(3)]
    inv[mid] = {
        "glb": str(out.relative_to(ROOT)),
        "bbox_m": dims,
        "triangles": tris,
        "bytes": out.stat().st_size,
    }
    print(json.dumps({"id": mid, "bbox_m_zup": dims, "tris": tris, "bytes": out.stat().st_size}), flush=True)
