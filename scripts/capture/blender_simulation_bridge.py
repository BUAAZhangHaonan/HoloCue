...
def make_material():
    ...
    material.use_nodes=True;material.blend_method='BLEND'
    material.use_screen_refraction=False
    material.diffuse_color=(.12,.80,.64,1.)
    nodes=material.node_tree.nodes;links=material.node_tree.links;nodes.clear()
    output=nodes.new('ShaderNodeOutputMaterial')
    mix=nodes.new('ShaderNodeMixShader')
    transparent=nodes.new('ShaderNodeBsdfTransparent')
    emission=nodes.new('ShaderNodeEmission');emission.inputs['Color'].default_value=(.12,.80,.64,1.)
    image=nodes.new('ShaderNodeTexImage')
    image.image=bpy.data.images.load(str(ROOT/'assets/ui/gaussian_sprite.png'),check_existing=True)
    vertex=nodes.new('ShaderNodeAttribute');vertex.attribute_name='Opacity'
...
class Consumer:
    def __init__(self,path):
        self.path=path;self.material=make_material();self.cues={};self.last_stamp=-1.
    ...
    def update(self):
        ...
        scene.render.pixel_aspect_x=max(1.,pixel_ratio)
        scene.render.pixel_aspect_y=max(1.,1./pixel_ratio)
        position=Vector(data['camera']['position_m']);look=Vector(data['camera']['look_at_m'])
        camera.location=position;camera.rotation_mode='QUATERNION'
        forward=np.asarray(look-position,dtype=float);forward/=np.linalg.norm(forward)
        ...
        for oid,pose in data['object_poses'].items():
            parent=bpy.data.objects['HC_OBJECT_'+oid];parent.matrix_world=pose_matrix(pose)
        ...
        for parent in list(bpy.data.objects):
            if parent.name.startswith('HC_ENV_'):
                for child in descendants(parent):
                    child.hide_render=inspection;child.hide_viewport=inspection
        keep={cue['task_id'] for cue in data['cues']}
        for tid in list(self.cues):
            if tid not in keep:
                obj=self.cues.pop(tid);mesh=obj.data
        ...
        for cue in data['cues']:
            tid=cue['task_id'];count=cue['n_gaussians']
            center=np.asarray(cue['centers_m'],float);radius=np.asarray(cue['radii_m'],float)
            ...
            if tid not in self.cues or len(self.cues[tid].data.vertices)!=4*count:
                ...
                obj=bpy.data.objects.new('HC_CUE_'+tid,mesh);scene.collection.objects.link(obj)
                obj.data.materials.append(self.material.copy());self.cues[tid]=obj
            obj=self.cues[tid];obj.data.vertices.foreach_set('co',vertices.ravel())
            ...
            if cue_color.shape!=(3,) or not np.isfinite(cue_color).all() or np.any((cue_color<0)|(cue_color>1)):
                raise ValueError('cue color must contain three normalized linear components')
            obj.data.materials[0].node_tree.nodes['Emission'].inputs['Color'].default_value=(*cue_color,1.)
            obj.data.update()
...
def verify_consumed(consumer,data):
    ...
    matrices={oid:np.asarray(bpy.data.objects['HC_OBJECT_'+oid].matrix_world,dtype=float)
              for oid in data['object_poses']}
    object_error=max(float(np.max(np.abs(matrices[oid]-np.asarray(pose_matrix(pose)))))
                     for oid,pose in data['object_poses'].items())
    errors=[]
    for cue in data['cues']:
        ...
        result={'task_id':cue['task_id'],'count':len(vertices),
        ...
        if max(result[k] for k in ('center_error_m','radius_error_m','opacity_error','color_error'))>2e-5:
    ...
    native_forward=np.asarray(scene.camera.rotation_quaternion @ Vector((0.,0.,-1.)))
    native_up=np.asarray(scene.camera.rotation_quaternion @ Vector((0.,1.,0.)))
    ...
    native_projection=np.asarray(scene.camera.calc_matrix_camera(
        bpy.context.evaluated_depsgraph_get(),
        x=scene.render.resolution_x,y=scene.render.resolution_y,
        scale_x=scene.render.pixel_aspect_x,scale_y=scene.render.pixel_aspect_y),dtype=float)
    expected_y=1./np.tan(data['camera']['fov_rad']/2.)
    ...
    return {'blender_version':bpy.app.version_string,'session_id':data['session_id'],
            'revision':data['revision'],'epoch':data['epoch'],'camera_error_m':camera_error,
            'camera_forward_error':forward_error,'camera_up_error':up_error,
            ...
            'camera_native_forward':native_forward.tolist(),'camera_native_up':native_up.tolist(),
            'camera_fov_rad':scene.camera.data.angle_y,'object_matrix_error':object_error,
            'object_matrices':{k:v.tolist() for k,v in matrices.items()},'cues':errors}
...
def inspect_native_capture(directory):
    ...
    for layer in manifest['passes']:
        ...
        alpha=pixels[:,3];rgb=pixels[:,:3]
        colors=list(layer['material_colors'].values())
        record={'path':str(path),'sha256':digest(path),'size':list(image.size),
                'alpha_mode':image.alpha_mode,'color_space':image.colorspace_settings.name,
                'alpha_min':float(alpha.min()),'alpha_max':float(alpha.max()),
                'nonzero_alpha_pixels':int((alpha>1e-6).sum()),
                'positive_alpha_black_rgb_pixels':int(((alpha>.01)&(np.max(rgb,axis=1)<1e-5)).sum()),
                'thresholds':[]}
        for threshold in (.001,.01,.1,.5):
            mask=alpha>threshold
            if not mask.any():
                continue
            unpremult=rgb[mask]/alpha[mask,None]
            row={'alpha_greater_than':threshold,'count':int(mask.sum()),
                 'unpremult_rgb_quantiles':np.quantile(unpremult,[0,.01,.1,.5,.9,.99,1],axis=0).tolist()}
            if all(color==colors[0] for color in colors):
                ...
                row['premult_residual_quantiles']=np.quantile(residual[mask],[0,.01,.5,.99,1],axis=0).tolist()
...
def native_layer_plan(consumer,data):
    ...
    scene=bpy.context.scene
    forward=np.asarray(scene.camera.rotation_quaternion @ Vector((0.,0.,-1.)),dtype=float)
    camera=np.asarray(scene.camera.location,dtype=float)
    sources={};ordered=[];hidden=[]
    for cue in data['cues']:
        tid=cue['task_id'];obj=consumer.cues[tid];mesh=obj.data
        ...
        sources[tid]={'vertices':vertices,'opacity':opacity.reshape(count,4),'material':mesh.materials[0]}
        if obj.hide_render:
            hidden.append({'task_id':tid,'count':count,'reason':'live visibility contract'})
            continue
        depth=(vertices-camera) @ forward
        if np.max(np.ptp(depth,axis=1),initial=0.)>2e-5:
            raise AssertionError('Sprites are not camera parallel; whole-quad depth ordering is invalid')
        ordered.extend({'task_id':tid,'gaussian_index':index,'camera_depth_m':float(depth[index].mean())}
...
def native_chunk(sources,members,index):
    ...
    if not np.array_equal(actual_vertices,vertices.ravel()) or not np.array_equal(actual_opacity,opacity.ravel()):
    ...
    evidence={'count':count,'members':members,
              ...
              'material_colors':{tid:list(sources[tid]['material'].node_tree.nodes['Emission'].inputs['Color'].default_value)
...
def coplanar_conflict_groups(vertices,depths,right,up):
    ...
    lo=xy.min(axis=1);hi=xy.max(axis=1)
    ...
    for index in range(count):
        conflict[index]=((np.abs(depths-depths[index])<=tolerance) &
                         (lo[index,0]<hi[:,0]) & (hi[index,0]>lo[:,0]) &
                         (lo[index,1]<hi[:,1]) & (hi[index,1]>lo[:,1]))
    np.fill_diagonal(conflict,False)
    degree=conflict.sum(axis=1)
    if degree.any():
        colors=np.full(count,-1,dtype=int);sizes=[]
        for index in np.argsort(-degree,kind='stable'):
            blocked=set(colors[conflict[index]].tolist())
            color=next((k for k,size in enumerate(sizes)
                        if k not in blocked and size<NATIVE_LAYER_LIMIT),len(sizes))
            if color==len(sizes):sizes.append(0)
            colors[index]=color;sizes[color]+=1
    ...
    return groups,{'grouping_method':method,'coplanar_depth_tolerance_m':tolerance,
                   'coplanar_conflict_edges':int(degree.sum()//2),
                   'coplanar_conflict_max_degree':int(degree.max()),
                   'color_run_quad_count':count,'color_run_pass_count':len(groups),
...
def native_pass_groups(sources,ordered):
    ...
    """
    def color(row):
        return tuple(sources[row['task_id']]['material'].node_tree.nodes['Emission'].inputs['Color'].default_value[:3])
    camera=bpy.context.scene.camera
    right=np.asarray(camera.rotation_quaternion @ Vector((1.,0.,0.)),dtype=float)
    up=np.asarray(camera.rotation_quaternion @ Vector((0.,1.,0.)),dtype=float)
    groups=[];start=0;run_index=0
    while start<len(ordered):
        end=start+1;rgb=color(ordered[start])
        ...
        for phase,selected in enumerate(indices):
            groups.append(([run[index] for index in selected],
                           {'color_run':run_index,'emission_rgb':list(rgb),
                            'color_run_pass_index':phase,**evidence}))
        start=end;run_index+=1
...
def capture_layered(consumer,data,destination,index):
    ...
    geometry=[obj for obj in scene.objects if obj.name not in cue_names
              and obj.type in ('MESH','CURVE','SURFACE','META','FONT','VOLUME')]
    ...
    loaded_images=[];current_chunk=None
    manifest={'method':'native Cycles bounded Gaussian passes with native compositor AlphaOver',
              'chunk_limit':NATIVE_LAYER_LIMIT,'samples':scene.cycles.samples,
              ...
              'base_denoising':old_denoising,'cue_denoising':False,
              ...
              'all_passes_coplanar_overlap_free_verified':all(group[1]['coplanar_overlap_free_verified'] for group in groups),
              'compositor_batch_layer_limit':16,
              'compositor_max_loaded_input_images':17,
              'compositor_max_input_rgba_float32_bytes':17*scene.render.resolution_x*scene.render.resolution_y*16,
              'compositor_batches':[],
              ...
              'visible_gaussians':len(ordered),'hidden_cues':hidden,
              ...
              'view_transform':scene.view_settings.view_transform,'view_look':scene.view_settings.look,
    ...
    try:
        ...
        def image_node(path):
            ...
            node=nodes.new('CompositorNodeImage');node.image=image
        ...
        for batch_index,start in enumerate(batch_starts):
            ...
            composite=nodes.new('CompositorNodeComposite');links.new(result,composite.inputs['Image'])
            final=start+len(batch)>=len(layers)
            output=(destination/f'frame_{index:04d}.exr' if final else
                    directory/f'composite_batch_{batch_index:04d}.exr')
            ...
            manifest['compositor_batches'].append({'index':batch_index,
                'input_accumulator':str(accumulator),'input_accumulator_sha256':digest(accumulator),
                'layer_paths':[row['path'] for row in batch],
                'loaded_input_images':len(loaded_images),'output':result_file})
    ...
    return {'manifest':str(manifest_path),'manifest_sha256':digest(manifest_path),
            'visible_gaussians':len(ordered),'native_pass_count':len(manifest['passes']),
            'full_native_geometry_unchanged':True,'composite_exr':manifest['composite_exr'],
            'composite_png':manifest['composite_png']}


def main():
    parser=argparse.ArgumentParser();parser.add_argument('--frame-file',type=Path,required=True)
    parser.add_argument('--capture',type=Path);parser.add_argument('--frames',type=int,default=12)
    ...
    device_record={'device':args.device,'cuda_visible_devices':os.environ.get('CUDA_VISIBLE_DEVICES','')}
    if args.device=='CUDA':
        ...
        device_record['devices']=[{'id':item.id,'name':item.name,'type':item.type,'use':item.use}
                                  for item in preferences.devices]
    else:
    ...
    if args.capture:
        ...
        for index in range(args.frames):
            ...
            (args.capture/f'consumed_frame_{index:04d}.json').write_text(json.dumps(data,ensure_ascii=False))
