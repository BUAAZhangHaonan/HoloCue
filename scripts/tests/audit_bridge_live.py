...
def check_environment(output):
    ...
    for name in ("TMPDIR", "TMP", "TEMP", "XDG_CACHE_HOME", "PIP_CACHE_DIR", "PLAYWRIGHT_BROWSERS_PATH"):
        ...
        if not value.is_relative_to(project / ".work") and not value.is_relative_to(
            project / "runs/simulation"
        ):
            raise ValueError(f"{name} must resolve inside project .work or runs/simulation")
        settings[name] = str(value)
    if not output.is_relative_to(project / "runs/simulation") and not output.is_relative_to(
        project / ".work"
    ):
...
class Audit:
    ...
    def view(self, revision=None, mode=None, after=0.0, selected=None):
        deadline = time.monotonic() + 25
        while time.monotonic() < deadline:
            paths = (
                [self.view_path]
                if self.view_path
                else list((root() / "runs/simulation/viewer").glob("client_*_view.json"))
            )
            for path in paths:
                if not path.is_file():
                    continue
    ...
    def control(self, name, execution=None):
        ...
        data = self.wait_state(
            lambda s: (
                s["revision"] > before["revision"] and (execution is None or s["execution"] == execution)
            )
        )
    ...
    def select_view(self, mode, target=None):
        ...
        self.page.get_by_role(
            "button",
            name={"workspace": "工作区域", "detail": "目标特写", "inspection": "结构检查"}[mode],
            exact=True,
        ).click()
        ...
        while True:
            ...
            check["stable_camera_seconds"] = 0.0 if stable_since is None else now - stable_since
            check["view_generated_at"] = view["generated_at"]
            check["distinct_camera_exports"] = len(camera_exports)
            camera_checks.append(check)
            previous_camera = camera
            if (
                check["passed"] and check["stable_camera_seconds"] >= 0.75 and len(camera_exports) >= 3
            ) or now > deadline:
                break
            self.page.wait_for_timeout(100)
        ...
        if not check["passed"] or check["stable_camera_seconds"] < 0.75 or len(camera_exports) < 3:
        ...
        local = np.asarray(
            obj.interaction.inspect_point_local_m if inspection_focus else obj.interaction.cue_offset_local_m,
            dtype=float,
        )
        if inspection_focus:
            local = (
        ...
        while time.monotonic() < deadline:
            ...
            save(
                self.out / "focus_failure.json",
                {
                    "expected_depth_m": expected_focus,
                    "before_focus_m": before_focus,
                    "observed_view": view,
                    "focus_point_m": point.tolist(),
                    "camera_before_focus": camera.tolist(),
                    "snapshot": snapshot,
                },
            )
            raise AssertionError("Visible focus control did not focus the contracted target plane")
        self.focus_actions.append(
            {
                "time": time.time(),
                "mode": mode,
                "target": obj.object_id,
                "previous_focus_m": before_focus,
                "expected_depth_m": expected_focus,
                "observed_focus_m": view["focus_m"],
                "control": "聚焦选中对象",
    ...
    def open(self):
        ...
        self.view()
        save(
            self.out / "session.json",
            {
                "session_id": self.sid,
                "scene_id": self.spec.scene_id,
                "client_view_path": str(self.view_path),
                "created_through": "Viser scene dropdown",
            },
        )
    ...
    def start_exporter(self):
        command = [
            sys.executable,
            "scripts/scenes/export_live_frames.py",
            "--session",
            self.sid,
            "--api",
            self.args.api,
    ...
    def wait_frame(self, snapshot, view):
        ...
        while time.monotonic() < deadline:
            ...
            if path.is_file():
                ...
                if (
                    (data["session_id"], data["revision"], data["epoch"])
                    == (self.sid, state["revision"], state["epoch"])
                    and data["generated_at"] >= view["generated_at"]
                    and data["view_mode"] == view["view_mode"]
                    and data["selected_id"] == view["selected_id"]
                ):
                    return path, data
            self.page.wait_for_timeout(100)
    ...
    def browser_sample(self, directory, index, reference):
        ...
        filename = f"viser_sample_{index:02d}.png"
        started = time.time()
        ...
        bounds = (
            int(np.floor(box["x"])),
            int(np.floor(box["y"])),
            int(np.ceil(box["x"] + box["width"])),
            int(np.ceil(box["y"] + box["height"])),
        )
    ...
    def settle_browser_capture(self, directory, reference, first):
        ...
        for index in range(1, 6):
            ...
            stable = (
                previous["dimensions_stable"]
                and current["dimensions_stable"]
                and previous["dimensions_after"] == current["dimensions_before"]
                and previous["canvas_sha256"] == current["canvas_sha256"]
            )
            save(
                directory / "browser_capture.json",
                {
                    "passed": stable,
                    "samples": records,
    ...
    def capture(self, name, mode=None, target=None):
        ...
        save(
            ...
            {
                "passed": False,
                "samples": [first_browser_sample],
                "scope": "Awaiting native render and a subsequent matching real canvas capture",
            },
        )
        command = [
            sys.executable,
            "scripts/guard/resource_guard.py",
            "--gpus",
            "2",
            "--rss-limit-gb",
            "12",
            "--log",
            str(directory / "resource_guard.jsonl"),
            "--execute",
            "--",
            str(self.blender),
            "-b",
            str(root() / "scenes" / self.spec.scene_id / "scene.blend"),
            "-t",
            "4",
            "--python-exit-code",
            "1",
            "--python",
            "scripts/capture/blender_simulation_bridge.py",
            "--",
            "--frame-file",
            str(frame_path),
            "--capture",
        ...
        if (consumed["session_id"], consumed["revision"], consumed["epoch"]) != (
            self.sid,
            data["revision"],
            data["epoch"],
        ):
        ...
        for object_id, pose in data["object_poses"].items():
            if not np.allclose(
                matrix(Pose.model_validate(pose)),
                matrix(Pose.model_validate(consumed["object_poses"][object_id])),
                atol=1e-7,
            ):
    ...
    def wait_elapsed(self, task_id, minimum):
        ...
        while time.monotonic() < deadline:
            view = self.view()
            if view["elapsed_s"].get(task_id, 0.0) >= minimum:
                return view
                ...
                self.inspected.append(
                    {
                        ...
                        "view": mode,
                        "image": str(directory / f"{name}.png"),
                        "blender": str(self.out / ("object_" + name) / "frame_0000.png"),
    ...
    def workflow(self):
        ...
        expected = [
            (step.target_id, step.action, step.reference_id, step.angle_deg)
        ...
        if {
            task["semantic"]["target_id"]
            for task in initial_tasks
            if task["semantic"]["task_role"] == "background"
        } != set(self.spec.task_contract.persistent_targets):
            raise AssertionError("Real model persistent targets differ from scene contract")
        self.control("暂停动作", "paused")
        first = actionable[0]
        duration = next(
            obj for obj in self.spec.objects if obj.object_id == first["semantic"]["target_id"]
        ).interaction.duration_s
        start_view = self.view()
        elapsed = start_view["elapsed_s"].get(first["task_id"], 0.0)
        save(
            self.out / "start_timing.json",
            {
        ...
        if (
            temporary["semantic"]["target_id"] != self.spec.task_contract.interrupt_target
            or temporary["semantic"]["action"] != "inspect_back"
        ):
        ...
        restored_mode = (
            "inspection"
            if first_semantic["action"] == "inspect_back"
            else "workspace"
            if first_reference
            else "detail"
        )
        _, restored = self.capture("04_original_restored", restored_mode, first_semantic["target_id"])
        if not saved_clock <= restored["elapsed_s"].get(first["task_id"], 0.0) < duration:
            raise AssertionError("Resume reset the interrupted clock or skipped its remaining motion")
        self.control("恢复任务", "running")
        self.wait_elapsed(first["task_id"], duration + 0.2)
        self.control("暂停动作", "paused")
        end, _ = self.capture("05_endpoint_waiting")
        if end["state"]["queue"] != initial_tasks or any(
            t["task_id"] == first["task_id"] for t in end["state"]["completed"]
        ):
        ...
        for index, expected_signature in enumerate(expected):
            snapshot = self.snapshot()
            current = snapshot["state"]["queue"][0]
            ...
            if index and cue.action in ("rotate", "insert", "assemble"):
                ...
                started = start["elapsed_s"].get(cue.task_id, 0.0)
                if started >= cue.interaction.duration_s * 0.75:
                ...
                self.control("恢复任务", "running")
                self.wait_elapsed(cue.task_id, cue.interaction.duration_s + 0.2)
                self.control("暂停动作", "paused")
                end, _ = self.capture(f"step_{index:02d}_endpoint", motion_mode, cue.target_id)
                if cue.reference_id:
                    receiver, _ = self.capture(
                        f"step_{index:02d}_endpoint_receiver", "detail", cue.reference_id
                    )
                    if receiver["state"] != end["state"]:
                        raise AssertionError("Receiver observation changed the paused task state")
            elif index:
        ...
        if len(commits) < 2 or any(
            not row.get("raw_response") or row.get("backend_mode") != "live" for row in commits
        ):
...
def main():
    ...
    results = []
    with (
        httpx.Client(base_url=args.api, headers=headers, timeout=10) as http,
        ...
        try:
            for scene_id in selected:
                ...
                directory = args.out / scene_id
                directory.mkdir()
                context = browser.new_context(
                    viewport={"width": 1600, "height": 1000}, record_video_dir=str(directory / "video")
                )
                page = context.new_page()
                audit = Audit(page, http, spec, directory, args, blender)
                try:
                    ...
                    if audit.sid:
                        try:
                            save(
                                directory / "events_on_failure.json",
                                audit.read(f"/api/v1/sessions/{audit.sid}/events"),
                            )
