...
def preflight(args):
    ...
    for name in ('TMPDIR', 'TMP', 'TEMP', 'XDG_CACHE_HOME', 'PIP_CACHE_DIR',
                 'PLAYWRIGHT_BROWSERS_PATH'):
        value = os.environ.get(name)
        if not value:
            raise ValueError(f'{name} must be set before running the audit')
        location = inside(value, project)
        if not (location.is_relative_to(project / '.work') or
                location.is_relative_to(project / 'runs' / 'simulation')):
...
class Audit:
    def __init__(self, args, http):
        ...
        self.report = {'passed': False, 'full_live_fault_coverage': False,
                       'started_at': time.time(), 'checks': {},
                       'browser_errors': [], 'faults_enabled': args.faults,
                       'source_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                       'limitations': ['Stale display rejection is a production VersionGate replay '
    ...
    def send(self, page, sid, text):
        ...
        def measure(name, action):
            ...
            try:
                ...
                timing['phases'].append({'name': name, 'started_at': start,
        ...
        try:
            ...
            events = measure('event_collection', lambda: self.until(
                'new real model job',
                lambda: self.run.request('GET', f'/api/v1/sessions/{sid}/events'),
                lambda rows: any(row['kind'] == 'turn_started' and
                                 row['data']['epoch'] > before for row in rows)))
            event = next(row for row in reversed(events) if row['kind'] == 'turn_started' and
                         row['data']['epoch'] > before)
    ...
    def job(self, jid, expected='done'):
        job = self.until('model job terminal state',
                         lambda: self.run.request('GET', f'/api/v1/jobs/{jid}'),
                         lambda row: row['status'] != 'planning', self.args.model_timeout)
        if job['status'] != expected:
            raise AssertionError(f'Expected {expected}, received {job}')
        if expected == 'done' and (job['data'].get('backend_mode') != 'live' or
                                   not job['data'].get('raw_response')):
    ...
    def control(self, page, sid, button, execution):
        ...
        return self.until(button, lambda: self.state(sid),
                          lambda row: row['revision'] > revision and row['execution'] == execution)

    def frozen(self, sid, duration=.8):
    ...
    def establish(self, browser):
        ...
        for index, scene_id in enumerate(self.args.scenes):
            ...
            context = browser.new_context(viewport={'width': 1440 - index * 340, 'height': 900},
            ...
            page.on('pageerror', lambda error, i=index: self.report['browser_errors'].append(
            ...
            actual = [signature(task) for task in state['queue']
                      if task['semantic']['task_role'] != 'background']
            expected = [(s.target_id, s.action, s.reference_id, s.angle_deg)
            ...
            self.checkpoint(f'client_{index}_established', {'session_id': sid, 'scene_id': scene_id,
                                                          'real_model_job': jid, 'state': state})
        self.checkpoint('two_independent_contexts', {'sessions': [item[0] for item in sessions],
    ...
    def isolation(self, sessions):
        ...
        for index in (0, 1):
            ...
            self.tab(page, '显示响应')
            focus = min(19., changed['focus_m'] + .35)
            self.field(page, '焦点数值 米').fill(str(focus))
            self.field(page, '焦点数值 米').press('Tab')
            changed = self.view(sid, time.time(), lambda row: abs(row['focus_m']-focus) < .001)
            ...
            keys = ('session_id', 'scene_id', 'revision', 'epoch', 'position_m', 'look_at_m',
                    'focus_m', 'view_mode', 'elapsed_s')
            ...
            evidence.append({'operated': sid, 'protected': other, 'changed_view': changed,
                             'protected_before': protected_view, 'protected_after': after_other})
    ...
    def invalidation(self, sid, spec):
        ...
        self.checkpoint('inflight_pause_and_display_replay', {
            'planning': planning, 'paused': paused, 'cancelled_job': cancelled,
            'frozen': frozen, 'old_snapshot': old, 'latest_snapshot': latest,
            'stale_replay_rejected': rejected, 'scope': 'production VersionGate real-packet replay'})
        # Explicit protocol negative probe. Normal operator behavior stays on UI controls.
        response = self.run.client.post(f'/api/v1/sessions/{sid}/control/resume',
                                        json={'expected_revision': old['state']['revision']})
        probe = {'status': response.status_code, 'body': response.text,
        ...
        self.checkpoint('rapid_real_instructions', {'superseded': old_job, 'committed': new_job,
                                                   'final_state': state, 'frozen': self.frozen(sid)})
    ...
    def helper(self, helper, *arguments, overrides=None):
        ...
        command = ['bash', '-c', 'set -e; source "$1"; shift; exec "$@"', 'holocue-fault-audit',
                   str(task_env), 'env', *(overrides or []), sys.executable, str(helper), *arguments]
        completed = subprocess.run(command, cwd=project, capture_output=True, text=True, timeout=65)
        self.counter += 1
        save(self.args.out / f'fault_command_{self.counter}.json', {
    ...
    def stop_api(self, name):
        ...
        if psutil.pid_exists(identity['pid']):
            ...
            if (process.create_time() == identity['create_time'] and
    ...
    def launch_api(self, suffix, model_url=None):
        ...
        self.helper(self.args.launch_helper, name, '--', 'bash', 'scripts/run_simulation.sh', 'api',
                    overrides=overrides)
    ...
    def stop_test_apis(self):
        ...
        self.report['current_owned_api_service'] = None
        self.report['external_api_restart_required'] = self.report.get(
            'external_api_restart_required', False)
        self.fault_recovery_needed = False
        save(self.args.out / 'report.json', self.report)
    ...
    def faults(self, sid, spec):
        ...
        self.control(page, sid, '暂停动作', 'paused')
        self.checkpoint('owned_api_restart_explicit_continue', {
            'stopped_identity': stopped, 'prior': prior, 'recovered': recovered,
            'offline': offline, 'before_explicit_continue': frozen, 'continued': continued})
        ...
        with socket.socket() as probe:
            ...
            try:
                ...
                if (failed['execution'] != 'error' or
                        'ConnectError:' not in failure['data'].get('error', '') or
                        'ConnectError:' not in (failed['last_error'] or '')):
        ...
        self.control(page, sid, '暂停动作', 'paused')
        self.checkpoint('real_model_connection_failure_and_explicit_retry', {
            'fault_model_url': invalid_url, 'failed_job': failure, 'failed_state': failed,
            'frozen': motion, 'restored_before_operator': unchanged, 'explicit_retry_job': success})
...
def main():
    ...
    options = {'headless': True, 'args': ['--use-gl=angle', '--use-angle=swiftshader',
            ...
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    if args.faults:
                        ...
                        audit.report['limitations'].append('API restart and model failure not run; enable --faults.')
