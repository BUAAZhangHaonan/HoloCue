...
def reflection(runtime,path):
    ...
    indices=np.flatnonzero(np.einsum('ij,ij->i',mesh.face_normals,eye)>.85)
    ...
    uv=np.column_stack([np.arctan2(local[:,2],local[:,0])/(2*np.pi)+.5,np.arcsin(local[:,1])/np.pi+.5])
    return {'front_triangles':indices.tolist(),'reflected_three_world':direction.tolist(),
            'environment_local_uv':uv.tolist(), 'mapping_note':'Geometric direction estimate; native decoded HDR and actual rendered pixels are authoritative.'}
...
def main():
    ...
    ready=threading.Event();renderers=[];errors=[];orientation=[(1.,0.,0.,0.)]
    @server.on_client_connect
    def connected(client):
        client_spec=spec.model_copy(deep=True)
        client_spec.render_hints.environment_wxyz=orientation[0]
        renderers.append(SceneRenderer(client,client_spec));ready.set()
    rows=[];environment_messages=[]
    def received(payload):
        size=int.from_bytes(payload[:8],'little');compressed=int.from_bytes(payload[8:16],'little')
        messages=msgpack.unpackb(zstandard.ZstdDecompressor().decompress(payload[16:16+compressed],max_output_size=size),raw=False,strict_map_key=False)['messages']
        for message in messages:
            if message.get('type')=='EnvironmentMapMessage':
                data=message['hdri_data'];sha=hashlib.sha256(data).hexdigest()
                (args.out/f'{sha}.jpg').write_bytes(data)
                environment_messages.append({**{k:v for k,v in message.items() if k!='hdri_data'},'hdri_sha256':sha,'hdri_bytes':len(data)})
    try:
        with sync_playwright() as pw:
            ...
            for state,snapshot in [('initial',initial),('completed',completed)]:
                ...
                for label,q in [('before',(1.,0.,0.,0.)),('after',spec.render_hints.environment_wxyz)]:
                    ...
                    if not ready.wait(30):raise TimeoutError('Viser client did not connect')
                    renderer=renderers[-1];renderer.guidance_enabled=False
                    renderer.update_packet(DisplayPacket.model_validate(snapshot['display']))
                    renderer.packet=None
                    renderer.select_view('detail','M')
                    renderer.client.flush();page.wait_for_timeout(5000)
                    runtime=page.evaluate(JS)
                    assert runtime['environment']['width']==1024 and runtime['environmentIntensity']==.1
                    assert len(runtime['meshes'])==1
                    assert runtime['meshes'][0]['metalness']==.95 and runtime['meshes'][0]['roughness']==.08
                    ...
                    if state=='completed' and label=='after':
                        for oid in ('L1','L2','TARGET'):
                            renderer.select_view('detail',oid);renderer.client.flush();page.wait_for_timeout(2000)
                            page.screenshot(path=str(args.out/f'{oid}_after.png'))
                    context.close()
                for key in pair[0]['camera']:
                    np.testing.assert_allclose(pair[0]['camera'][key],pair[1]['camera'][key],atol=1e-7,rtol=0)
                assert pair[0]['meshes']==pair[1]['meshes']
                rows.append({'state':state,'session_id':snapshot['state']['session_id'],'revision':snapshot['state']['revision'],'camera_and_materials_unchanged':True})
            browser.close()
    finally:
        server.stop()
        save(args.out/'report.json',{'pairs':rows,'browser_errors':errors,'guidance_enabled':False,
             'environment_messages':environment_messages,'render_pixel_ratio':'1.0 via visible Viser setting',
             'visual_acceptance':'pending human/agent image inspection','scope':'Native production SceneRenderer with recorded real-model states; separate from main Viewer UI workflow',
             'inputs':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [args.live/'snapshots.json',args.live/'completed_snapshots.json']}})
