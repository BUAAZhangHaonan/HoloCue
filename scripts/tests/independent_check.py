...
def main():
    ...
    parser.add_argument('--flat-rendered',action='store_true',
    ...
    manifests=[p for p in sorted((ROOT/'scenes').glob('*/scene.json')) if not p.parent.name.startswith('_')]
    if len(manifests)!=12:
        raise AssertionError('expected twelve scene manifests')
    files=[];totals={'scenes':12,'objects':0,'meshes':0,'triangles':0}
    for path in manifests:
        ...
        for item in scene['objects']+scene['environment']:
            ...
            files.append({'path':item['asset'],'sha256':hashlib.sha256(asset.read_bytes()).hexdigest(),'triangles':count})
            totals['triangles']+=count;totals['meshes']+=1
    ...
    if (audit.get('status')!='passed' or len(audit['results'])!=11 or
            any(not row.get('passed') or row['penetrations'] or
                any(part['status']=='invalid_solid_topology' for part in row['obstacle_coverage'])
    ...
    source_files+=list((ROOT/'scripts/tests').glob('*.py'))+list((ROOT/'scripts/capture').glob('*.py'))
    source_hashes=[]
    for path in sorted(source_files):
        tree=ast.parse(path.read_text(),filename=str(path))
        source_hashes.append({'path':path.relative_to(ROOT).as_posix(),
    ...
    failures=sum(int(suite.attrib['failures'])+int(suite.attrib['errors']) for suite in suites.iter('testsuite'))
    if failures:
        raise AssertionError('unit test evidence contains failures')
    report={'status':'passed','review_type':'independent_process','python':sys.version,
            'subagent_used':False,'native_blender_exercised':False,'native_viser_exercised':False,
            'model_generation_exercised':False,'tests':tests,'asset_totals':totals,'assets':files,
            'checked_source_files':source_hashes,
            'source_digest':hashlib.sha256(json.dumps(source_hashes,sort_keys=True).encode()).hexdigest()}
    report['evidence_inputs']={key:str(getattr(args,key).resolve()) for key in ('rendered','geometry','junit')}
    report['rendered_layout']='flat' if args.flat_rendered else 'scene_subdirectories'
    args.out.parent.mkdir(parents=True,exist_ok=True)
    args.out.write_text(json.dumps(report,indent=2))
    print(json.dumps({key:value for key,value in report.items() if key not in ('assets','checked_source_files')},indent=2))
