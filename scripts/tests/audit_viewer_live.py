...
def client_view_errors(client, view):
    ...
    box = canvas["box"]
    pairs = {
        "position_m": (position, view["position_m"]),
        "look_at_m": (target, view["look_at_m"]),
        "up_direction": (up, view["up_direction"]),
        "forward": (forward, expected_forward),
        "fov_rad": (camera["fov_rad"], view["fov_rad"]),
        "aspect": (camera["aspect"], view["aspect"]),
    }
    ...
    return {
        ...
        "errors": errors,
        "client_position_in_viser_world": position.tolist(),
        "client_target_in_viser_world": target.tolist(),
    }
...
def same_view(left, right, *, camera=True):
    ...
    return (
        all(left[key] == right[key] for key in VIEW_IDENTITY)
        and not left["clock_advancing"]
        and not right["clock_advancing"]
...
def same_client_frame(left, right):
    return (
        left.get("ready")
        and right.get("ready")
        and left["canvas"] == right["canvas"]
        and left["root"] == right["root"]
        and all(
            np.allclose(left["camera"][key], right["camera"][key], rtol=0, atol=1e-7)
            for key in left["camera"]
        )
    )
...
def canvas_pixels(path, box, viewport, clip=None):
    ...
    with Image.open(path) as screenshot:
        ...
        if clip is not None and all(
            abs(box[key] - clip[key]) <= 1e-9 for key in ("x", "y", "width", "height")
        ):
            ...
            bounds = (
                int(np.floor(edges[0])),
                int(np.floor(edges[1])),
                int(np.ceil(edges[2])),
                int(np.ceil(edges[3])),
            )
...
def assert_comparable_captures(left, right, *, changed_control=None, new_revision=False):
    ...
    for key in VIEW_NUMERIC:
        if key != changed_control and not np.allclose(
            left["view_after"][key], right["view_after"][key], rtol=0, atol=1e-7
        ):
...
class BrowserRun:
    ...
    def console_message(self, message):
        if message.type in ("warning", "error"):
            ...
            (self.out / "browser_console.json").write_text(
    ...
    def set_render_pixel_ratio(self, label):
        ...
        if value != label and not (
            label == "Adaptive" and value == "" and field.get_attribute("placeholder") == "Adaptive"
        ):
        ...
        self.record(
            "render_pixel_ratio_finished",
            requested=label,
            previous_visible_value=previous,
            selected_visible_value=value,
    ...
    def capture(self, name, *, stable=False, expected_response=None, **kwargs):
        ...
        try:
            ...
            (self.out / "capture_failure_views.json").write_text(
    ...
    def stable_capture(self, name, *, expected_response=None, **kwargs):
        ...
        def view():
            ...
            return wait_view(sid, time.time() - 0.5, revision=state["revision"], timeout=min(25, remaining))
        ...
        try:
            for index in range(6):
                ...
                self.page.screenshot(
                    path=str(self.out / filename), timeout=min(60000, remaining * 1000), **kwargs
                )
                sample.update(
                    finished_at=time.time(), client_after=self.page.evaluate(CLIENT_READ), view_after=view()
                )
                ...
                identity_fixed = (
                    all(before[key] == after[key] == reference[key] for key in VIEW_IDENTITY)
                    and before["elapsed_s"] == after["elapsed_s"] == reference["elapsed_s"]
                    and not before["clock_advancing"]
                    and not after["clock_advancing"]
                )
                response_fixed = all(
                    np.isclose(candidate[key], value, rtol=0, atol=1e-7)
                ...
                sample["pixels"] = settled_canvas_pixels(
                    self.out / filename, sample["client_before"], sample["client_after"], kwargs.get("clip")
                )
                if sample["pixels"] is None:
                    sample["pixel_comparison_skipped"] = (
                        "Client camera or canvas changed during screenshot; raw image retained."
                    )
                sample["stable_within_shot"] = (
                    identity_fixed
                    and response_fixed
                    and sample["api_state_unchanged"]
                    and sample["pixels"] is not None
                    and same_view(before, after)
                    and sample["client_view_before"]["passed"]
                    and sample["client_view_after"]["passed"]
                )
                matched = (
                    previous is not None
                    and previous["stable_within_shot"]
                    and sample["stable_within_shot"]
                ...
                if matched:
                    ...
                    report["accepted_file"] = name
                    report["accepted_sample"] = index
                    save()
                    return sample
                previous = sample
    ...
    def settled(self, sid, previous, planning=False):
        ...
        while time.monotonic() < deadline:
            ...
            if state["revision"] > previous and state["execution"] != "planning":
                ...
                if planning:
                    job_id = next(
                        e["data"]["job_id"] for e in reversed(events) if e["kind"] == "turn_started"
                    )
...
def main():
    ...
    parser.add_argument(
        "--scenes",
        nargs="+",
        choices=[entry["scene_id"] for entry in list_scenes()],
    ...
    with httpx.Client(base_url=args.api, timeout=10, headers=headers) as http:
        with sync_playwright() as playwright:
            ...
            for entry in entries:
                ...
                out = args.out / spec.scene_id
                out.mkdir(exist_ok=True)
                context = browser.new_context(
                    viewport={"width": 1600, "height": 1000}, record_video_dir=str(out / "video")
                )
                run = None
                try:
                    ...
                    software_notice = page.get_by_role("alert").filter(
                        has_text="Software WebGL rendering detected"
                    )
                    ...
                    while not ui_input(page, "会话标识").input_value():
                        if page.get_by_text("执行错误", exact=True).count():
                            (out / "initial_failure_dom.yaml").write_text(
                    ...
                    while True:
                        ...
                        if sid and run.state(sid)["scene_id"] == spec.scene_id:
                            break
                        if time.monotonic() > deadline:
                            (out / "scene_switch_failure_dom.yaml").write_text(
                                page.locator("body").aria_snapshot()
                            )
                    ...
                    expected = [
                        (s.target_id, s.action, s.reference_id, s.angle_deg)
                        for s in spec.task_contract.ordered_steps
                    ]
                    actual = [
                        signature(t) for t in state["queue"] if t["semantic"]["task_role"] != "background"
                    ]
                    if actual != expected:
                    ...
                    wait_view(sid, before, "detail")
                    run.capture("detail.png", stable=True)
                    ...
                    current = next(
                        o for o in spec.objects if o.object_id == state["queue"][0]["semantic"]["target_id"]
                    )
                    check_point = transform_points(
                        current.pose, np.asarray([current.interaction.inspect_point_local_m])
                    )[0]
                    check_normal = rotation(current.pose).apply(current.interaction.inspect_normal_local)
                    normal_side = float(
                        (np.asarray(response_view["position_m"]) - check_point) @ check_normal
                    )
                    if inspection_response and (
                        normal_side <= 0 or response_view["selected_id"] != current.object_id
                    ):
                        raise AssertionError("response camera is not on the contracted inspection side")
                    (out / "response_view_contract.json").write_text(
                        json.dumps(
                            {
                                "view_mode": response_mode,
                                "action": state["queue"][0]["semantic"]["action"],
                                "target_id": current.object_id,
                                "inspection_point_local_m": current.interaction.inspect_point_local_m,
                                ...
                                else "workspace includes task trajectory and receiver",
                                "view": response_view,
                            },
                            ensure_ascii=False,
                            indent=2,
                    ...
                    (out / "canvas_region.json").write_text(
                        json.dumps(
                            {
                                "canvas_box": canvas_box,
                                "viewport": page.viewport_size,
                                "canvas_html": canvas.evaluate("(element) => element.outerHTML"),
                            },
                            ensure_ascii=False,
                            indent=2,
                        )
                    )
                    zero_shot = run.capture(
                        "brightness_zero.png",
                        stable=True,
                        expected_response={"brightness": 0.0},
                        clip=canvas_box,
                    )
                    ...
                    wait_view(sid, before)
                    page.wait_for_timeout(400)
                    one_shot = run.capture(
                        "brightness_one.png",
                        stable=True,
                        expected_response={"brightness": 1.0},
                        clip=canvas_box,
                    )
                    ...
                    if changed < 20:
                        raise AssertionError(
                            f"{spec.scene_id}: brightness changed fewer than 20 canvas pixels"
                        )
                    focus = one_shot["view_after"]["focus_m"]
                    ui_input(page, "焦点数值 米").fill(str(round(focus + 0.35, 4)))
                    ui_input(page, "焦点数值 米").press("Tab")
                    before = time.time()
                    wait_view(sid, before)
                    page.wait_for_timeout(400)
                    defocused_shot = run.capture(
                        "defocused.png", stable=True, expected_response={"brightness": 1.0}, clip=canvas_box
                    )
                    ...
                    (out / "focus_comparison.json").write_text(
                        json.dumps(
                            {
                                "changed_pixels": focus_changed,
                                "metric_scope": "Whole-canvas changed pixels; not a locality or spatial-support measurement.",
                                "canvas_box": canvas_box,
                                "view": defocused_shot["view_after"],
                                "focused_shot": one_shot,
                                "defocused_shot": defocused_shot,
                            },
                            ensure_ascii=False,
                            indent=2,
                        )
                    ...
                    before = time.time()
                    interrupted = run.message(sid, spec.task_contract.interrupt_instruction)
                    if (
                        not interrupted["suspended"]
                        or [t["task_id"] for t in interrupted["suspended"][-1]] != original_ids
                    ):
                        raise AssertionError("GUI interruption did not retain original task identities")
                    temporary = [
                        t for t in interrupted["queue"] if t["semantic"]["task_role"] != "background"
                    ]
                    if len(temporary) != 1 or signature(temporary[0]) != (
                        spec.task_contract.interrupt_target,
                        "inspect_back",
                        None,
                        None,
                    ):
                        (out / "invalid_interruption.json").write_text(
                    ...
                    if [
                        signature(t) for t in resumed["queue"] if t["semantic"]["task_role"] != "background"
                    ] != actual:
                    ...
                    result = {
                        ...
                        "session_id": sid,
                        "backend_mode": "live",
                        "brightness_changed_pixels": changed,
                        "focus_changed_pixels": focus_changed,
                        "overview": overview,
                    ...
                    try:
                        (out / "failure_dom.yaml").write_text(
                            page.locator("body").aria_snapshot(timeout=5000)
                        )
                        if run is not None:
                            run.capture("failure.png")
    ...
    if len(results) != len(entries) or any(r["status"] != "passed" for r in results):
        raise AssertionError(
            f"scene audit incomplete or failed: {sum(r['status'] == 'passed' for r in results)}/{len(entries)} passed"
        )
