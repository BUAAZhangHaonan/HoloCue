...
class LiveRun:
    ...
    def request(self, method, path, **kwargs):
        ...
        (self.output / "http.json").write_text(
            json.dumps(self.steps, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    ...
    def message(self, sid, text):
        ...
        job = self.request(
            ...
            json={"text": text, "expected_revision": state["revision"], "request_id": uuid.uuid4().hex},
        )
...
def main():
    ...
    with httpx.Client(base_url=args.api, timeout=10, headers=headers) as client:
        ...
        for item in list_scenes():
            spec = load_scene(item["scene_id"])
            directory = args.out / spec.scene_id
            ...
            (directory / "initial_snapshot.json").write_text(
                json.dumps(initial_snapshot, ensure_ascii=False, indent=2), encoding="utf-8"
            )
            ...
            if len(state["queue"]) != 1 or signature(state["queue"][0]) != (
            ...
            if (
                completed_temporary["queue"]
                or completed_temporary["execution"] != "idle"
                or completed_temporary["suspended"][-1] != original_queue
            ):
                raise AssertionError("temporary completion did not leave the original plan suspended")
            (directory / "temporary_completed.json").write_text(
                json.dumps(completed_temporary, ensure_ascii=False, indent=2), encoding="utf-8"
            )
            ...
            for expected_task in expected:
                ...
                if not np.allclose(matrix(pose), matrix(endpoint), atol=1e-7):
                    raise AssertionError(
                        f"Explicit completion did not update {cue.target_id} to its endpoint"
                    )
                if state["queue"]:
            ...
            (directory / "snapshots.json").write_text(
                json.dumps(frames, ensure_ascii=False, indent=2), encoding="utf-8"
            )
            (directory / "completed_snapshots.json").write_text(
                json.dumps(completed_frames, ensure_ascii=False, indent=2), encoding="utf-8"
            )
