...
def main():
    ...
    rows=[];spec=load_scene('control_panel')
    instructions=[
        ('count_high','整体替换计划：检查 C 的背面，C 为当前操作，优先级 5，精确定位 precise；持续保留 B 的背景标签提示，B 优先级 1，persistent。',5000,1.),
        ('count_low','整体替换计划：仍检查 C 的背面，C 为当前操作，优先级 1，精确定位 precise；持续保留 B 的背景标签提示，B 优先级 5，persistent。',1000,1.),
        ('sigma_wide','整体替换计划：仍检查 C 的背面，C 为当前操作，优先级 1，显示需求明确使用普通上下文 neutral；持续保留 B 的背景标签提示，B 优先级 5，persistent。',1000,4.),
    ]
    with httpx.Client(base_url='http://127.0.0.1:8750',timeout=10) as http,sync_playwright() as p:
        browser=p.chromium.launch(headless=True,args=['--use-gl=angle','--use-angle=swiftshader','--enable-unsafe-swiftshader'])
        context=browser.new_context(viewport={'width':1600,'height':1000},record_video_dir=str(args.out/'video'))
        ...
        try:
            ...
            while True:
                ...
                if sid and run.state(sid)['scene_id']==spec.scene_id:break
                if time.monotonic()>deadline:raise TimeoutError('Scene selection did not create a session')
                page.wait_for_timeout(100)
            focus=None
            for name,instruction,count,sigma in instructions:
                planned=run.message(sid,instruction);state=run.control(sid,'pause')
                ...
                for packet in (state,snapshot['state'],snapshot['display']):
                    if (packet['session_id'],packet['scene_id'],packet['revision'],packet['epoch'],packet['execution'])!=(
                            sid,spec.scene_id,state['revision'],state['epoch'],'paused'):
                        raise AssertionError('Response experiment has inconsistent paused state/display identity')
                if state['revision']<=planned['revision'] or (rows and planned['revision']<=rows[-1]['view']['revision']):
                ...
                if (cue['action'],cue['task_role'],cue['n_gaussians'],cue['sigma_value'])!=('inspect_back','current',count,sigma):
                    raise AssertionError('Real model did not produce the requested response experiment: '+json.dumps(cue))
                ...
                if focus is None:
                    ...
                    focus=float(f"{round(view['focus_m']/.005)*.005:.3f}")
                    run.record('fixed_camera_selected',target_id='C',mode='inspection',
                               automatic_focus_m=view['focus_m'],requested_focus_m=focus)
                ...
                ui_input(page,'焦点数值 米').fill(str(focus));ui_input(page,'焦点数值 米').press('Tab')
                ui_input(page,'亮度数值').fill('1');ui_input(page,'亮度数值').press('Tab')
                deadline=time.monotonic()+10
                while True:
                    view=wait_view(sid,time.time(),'inspection',revision=state['revision'])
                    if (np.isclose(view['focus_m'],focus,rtol=0,atol=1e-9)
                            and np.isclose(view['brightness'],1.,rtol=0,atol=1e-9)):
                        break
                    if time.monotonic()>deadline:
                        raise AssertionError('Visible focus/brightness controls did not settle: '+json.dumps(view))
                    page.wait_for_timeout(100)
                ...
                if (view['session_id'],view['scene_id'],view['revision'],view['epoch'],view['execution'],
                        view['view_mode'],view['selected_id'],view['clock_advancing'])!=(
                        sid,spec.scene_id,state['revision'],state['epoch'],'paused','inspection','C',False):
                    raise AssertionError('Response comparison view is not the current paused C inspection')
                if not np.isclose(view['focus_m'],focus) or not np.isclose(view['brightness'],1.):
                    raise AssertionError('Response comparison did not apply the fixed focus and brightness')
                run.record('fixed_camera_response',name=name,revision=view['revision'],epoch=view['epoch'],
                           mode=view['view_mode'],selected_id=view['selected_id'],view=view)
                ...
                shot=run.capture(f'{name}.png',stable=True,
                                 expected_response={'focus_m':focus,'brightness':1.},clip=canvas)
                view=shot['view_after']
                row={'name':name,'instruction':instruction,'session_id':sid,'view':view,'snapshot':snapshot,
                     'planned_revision':planned['revision'],'follow_current_step':False,
                     'camera_setup': 'initial_selection' if not rows else 'reuse_without_refit',
                     'canvas_box':canvas,'viewport':page.viewport_size,'stable_shot':shot}
                rows.append(row)
                (args.out/f'{name}.json').write_text(json.dumps(row,ensure_ascii=False,indent=2))
            arrays=[np.asarray(Image.open(args.out/f'{row["name"]}.png').convert('RGB')).astype(int) for row in rows]
            differences=[]
            for left,right in ((0,1),(1,2)):
                assert_comparable_captures(rows[left]['stable_shot'],rows[right]['stable_shot'],new_revision=True)
                ...
                if changed<20:raise AssertionError('N/sigma did not change the native canvas')
                differences.append({'from':rows[left]['name'],'to':rows[right]['name'],'changed_pixels':changed,
                    'metric_scope':'Whole-canvas changed pixels; not a locality or spatial-support measurement.'})
            if errors:raise AssertionError(errors)
            (args.out/'report.json').write_text(json.dumps({'passed':True,'session_id':sid,'differences':differences,
                'browser_errors':errors,'parameter_source':['configs/display_policy.json','configs/preview_response.json'],
                'render_pixel_ratio':'1.0 selected through the existing visible Viser setting; product default remains Adaptive',
                'method':'Real model priority/depth requirements; native fixed-camera inspection controls'},ensure_ascii=False,indent=2))
        finally:
