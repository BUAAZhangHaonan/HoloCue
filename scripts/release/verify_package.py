...
for line in manifest.read_text(encoding='utf-8').splitlines():
    ...
    if not path.is_relative_to(ROOT):raise ValueError('Manifest path escapes project root')
    if not path.is_file():errors.append('missing '+relative)
    elif hashlib.sha256(path.read_bytes()).hexdigest()!=expected:errors.append('changed '+relative)
    count+=1
