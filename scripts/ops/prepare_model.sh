...
SIZE="${1:-4B}"
case "$SIZE" in 4B | 9B) ;; *)
  echo 'Only Qwen3.5-4B and Qwen3.5-9B are authorized'
  exit 2
  ;;
esac
...
if ((FREE_KB < 50 * 1024 * 1024)); then
  echo 'Require 50 GiB free disk before model preparation'
  exit 3
fi
if [[ "${MODEL_SOURCE:-}" == 'ssh' ]]; then
  ...
  case "$SOURCE_MODEL_PATH" in /home/g203-4028/Models/*) ;; *)
    echo 'Unexpected source root'
    exit 4
    ;;
  esac
  ...
  command -v hf > /dev/null || {
    echo 'Install huggingface_hub in the isolated model environment to provide hf'
    exit 5
  }
  ...
  echo 'Choose MODEL_SOURCE=ssh, hub, or local explicitly. No automatic source switching.'
  exit 6
fi
