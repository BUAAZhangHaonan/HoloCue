...
report = {
    ...
    "disk_gib": {"free": disk.free / 1024**3},
    ...
    "tools": {
        x: shutil.which(x)
        for x in ["uv", "python3.11", "python3.12", "blender", "rsync", "ssh", "nvidia-smi"]
    },
    "existing_project_model_dirs": [str(x) for x in (root / "models").glob("*")],
}
if shutil.which("blender"):
    report["blender_version"] = command(["blender", "--version"])
p = root / "runs/server_inventory.json"
